...
> **All phases MUST be deployed as a single atomic release.** There must be no window where old row-creation code runs against new status-evaluation logic or vice versa. The phases below are logical groupings for implementation and review, not sequential deployments.
...
- `src/app/registry/clearance/_schema/clearance.ts` — remove `emailSent` column (must align with SQL migration)
...
> All phases deploy as a single atomic release. The order below is for implementation and review sequencing only.

1. Create shared constants and consolidated status utilities (Phase 1).
2. Fix all status aggregation, workflow gating, `findPreviousClearedRequest`, `finalizeIfAllApproved`, ClearanceSwitch bug, service-level department validation, graduation detail TabsPanel bug, and `getStatusFromClearances` to use required departments only (Phase 2).
...
5. Remove Library, Resource, and LEAP from clearance and auto-approval UI, navigation, permissions, and service logic (Phase 5).
6. Run the data migration: delete pending Library rows, create `blocked_students` entries, drop `emailSent` column, delete library auto-approval records (Phase 6).
7. Clean up all text, labels, PDF content, quick-comment chips, history filters, and student-facing messages (Phase 7).
...
8. Registry clearance pages no longer present Library as a selectable or rendered department.
9. Library, Resource, and LEAP users no longer have clearance-specific navigation or approval capability.
10. Auto-approval creation and bulk import reject any attempt to target Library.
11. Pending counters and badge counts match the new required-department rules.
12. Historical approved/rejected Library data remains in the database for audit without affecting live workflows.
13. Clearance `department` column is immutable after creation (ClearanceSwitch bug fixed).
14. Service-level validation prevents cross-department approve/reject via direct API call.
15. Graduation clearance detail page Academics tab renders correctly (TabsPanel nesting fixed).
16. ClearanceHistory and GraduationClearanceHistory filter out library records from display.
17. `emailSent` column has been dropped from clearance table (migration verified).
18. Library auto-approval records deleted from database (migration verified).
19. `ClearanceSwitch` and `GraduationClearanceSwitch` no longer send `department` in the update payload.
20. Graduation Proof of Clearance PDF no longer mentions "library obligations".
21. "Library Fines Owing" chip is removed from clearance comments.
22. `getClearanceStatus` and `getOverallClearanceStatus` are consolidated into single shared utilities.
23. Editing an existing registration request does not recreate a Library clearance row.
24. `pnpm tsc --noEmit` and `pnpm lint:fix` pass with zero errors.
...
- Restrict valid clearance auto-approval departments to Finance only. Phase 6d migration deletes library auto-approval records from the database for a clean state.

### Risk: data migration fails on large datasets mid-transaction

Mitigation:

- The 6a deletion cascades through junction tables; test in a staging environment before production. The 6b insert uses a `NOT EXISTS` guard so it is safe to re-run.
