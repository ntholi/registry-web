...
Last update:
- Step 6 completed: remaining upload and delete flows now persist storage keys and use module-owned server actions with StoragePaths/uploadFile/deleteFile instead of deprecated client-side storage calls
...
1. **Proceed to Step 7**: update all retrieval code paths to resolve stored keys with `getPublicUrl()` and remove remaining assumptions that `fileUrl` already contains a public URL
2. **Verify there are no remaining hardcoded R2 public URLs**: grep for `pub-2b37ce26bd70421e9e59e4fe805c6873.r2.dev` and confirm there are no live call sites left before starting Step 7
3. **Keep live execution for downtime only**: run `pnpm tsx scripts/migrate-r2-storage.ts --phase all` and `pnpm tsx scripts/extract-base64-deposits.ts` during the maintenance window after deploying the Step 1/2/5/6/7 code changes
4. **Use the dry-run logs as the operator baseline**:
