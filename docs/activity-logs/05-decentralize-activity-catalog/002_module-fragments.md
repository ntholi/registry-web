...
Contains all registry-department activities (~40 entries):
...
Contains academic-department activities (~22 entries):
...
- Feedback (`feedback_cycle_created/updated`, `feedback_category_created/updated`, `feedback_question_created/updated`)

### 2.3 — Finance: `src/app/finance/_lib/activities.ts`

Contains finance-department activities (~6 entries):

- Payment receipts (`payment_receipt_added/removed`)
- Sponsors (`sponsor_created/updated`)
- Sponsorship (`sponsorship_assigned/updated`)

### 2.4 — Library: `src/app/library/_lib/activities.ts`

Contains library-department activities (~27 entries):

- Books (`book_added/updated/deleted`)
- Book copies (`book_copy_added/updated`)
- Loans (`book_loan_created`, `book_returned`, `loan_deleted`)
- Fines (`fine_created/updated`)
...
Contains marketing-department activities (~20 entries):

- Applications (`application_submitted/updated`, `application_status_changed`)
- Applicant documents (`applicant_document_uploaded/reviewed`)
- Deposits (`deposit_submitted/verified`)
...
Contains admin-department activities (~7 entries):

- Users (`user_created/updated/deleted`)
- Tasks (`task_created/updated`)
- Notifications (`notification_created/updated`)

### 2.7 — Timetable: `src/app/timetable/_lib/activities.ts`

Contains resource-department activities (~10 entries):

- Venues (`venue_created/updated`)
- Venue types (`venue_type_created/updated`)
...
- `tableOperationMap` values must reference keys from the same fragment's `catalog` — the `satisfies` constraint enforces this
