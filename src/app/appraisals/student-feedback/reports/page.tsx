...
import {
	...
	getStudentFeedbackReportData,
} from './_server/actions';

export default function StudentFeedbackReportsPage() {
	const [filter, setFilter] = useState<StudentFeedbackReportFilter>({});

	const { data: hasFullAccess = false } = useQuery({
		queryKey: ['student-feedback-report-access'],
	...
	const { data: reportData, isLoading } = useQuery({
		queryKey: ['student-feedback-report-data', filter],
		queryFn: () => getStudentFeedbackReportData(filter),
		enabled: isFilterApplied,
	});

	const handleFilterChange = useCallback(
		(newFilter: StudentFeedbackReportFilter) => {
			setFilter(newFilter);
		},
		[]
	);
