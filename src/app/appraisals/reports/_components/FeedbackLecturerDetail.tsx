...
export default function FeedbackLecturerDetail({ userId, filter }: Props) {
	const { data: detail, isLoading } = useQuery({
		queryKey: ['appraisal-feedback-lecturer-detail', userId, filter],
						...
						<Stack gap='sm'>
							<Group gap='sm'>
								<Badge
									size='lg'
									variant='light'
									color={ratingColor(detail.avgRating)}
								>
									Avg: {detail.avgRating.toFixed(2)}
								</Badge>
							</Group>
							<Text size='sm' c='dimmed'>
							...
							<Text size='sm' c='dimmed'>
								{detail.questions.reduce((s, q) => s + q.responseCount, 0)}{' '}
								total responses
							</Text>
						</Stack>
					</Grid.Col>
