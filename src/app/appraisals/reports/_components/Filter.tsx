...
export default function Filter({ onFilterChange, hasFullAccess }: Props) {
	...
	const { data: lecturers = [], isLoading: lecturersLoading } = useQuery({
		...
		enabled: hasFullAccess && debouncedSearch.length >= 2,
	});
				...
				{hasFullAccess && (
					<Grid.Col span={{ base: 12, sm: 6, md: 3 }}>
						<Select
							...
							data={lecturers}
							rightSection={lecturersLoading && <Loader size='xs' />}
							value={localFilter.lecturerId ?? null}
							onChange={(v) => handleChange('lecturerId', v)}
							searchable
							clearable
