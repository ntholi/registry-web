...
export const getApplicantDocument = createAction(async (id: string) =>
	applicantDocumentsService.get(id)
);

export const findDocumentsByApplicant = createAction(
	async (applicantId: string, page: number = 1) =>
		applicantDocumentsService.findByApplicant(applicantId, page)
);

export const findDocumentsByType = createAction(
	async (applicantId: string, type: DocumentType) =>
		applicantDocumentsService.findByType(applicantId, type)
);
...
export const uploadApplicantFile = createAction(
	async (applicantId: string, file: File): Promise<string> => {
...
export const deleteApplicantDocument = createAction(
	async (id: string, fileUrl: string) => {
		const linkedRecord = unwrap(
			await findAcademicRecordByApplicantDocumentId(id)
		);
		if (linkedRecord) {
			await unwrap(
...
export const analyzeDocumentWithAI = createAction(
	async (
		fileBase64: string,
		mediaType: string
	...
	async (
		...
		if (data.certificateNumber) {
			const existing = unwrap(
				await findAcademicRecordByCertificateNumber(data.certificateNumber)
			);
			if (existing) {
				const record = unwrap(
