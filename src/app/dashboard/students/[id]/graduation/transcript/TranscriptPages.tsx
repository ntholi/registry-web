...
const TermSection = ({
  ...
  return (
    <View style={tw('mb-2')}>
      ...
      {(semester.studentModules || []).map((module, i) => (
        <GradeRow
          key={i}
          courseCode={module.semesterModule?.module?.code || ''}
          courseName={module.semesterModule?.module?.name || ''}
          credits={module.semesterModule?.credits || 0}
          grade={module.grade || ''}
        />
      ))}
      <TermSummary
...
export default function TranscriptPages({
  student,
  studentIndex,
}: {
  student: Student;
  ...
  return (
    <>
      {completedPrograms.map((program, pIdx) => {
        ...
        return (
          <Fragment key={`student-${studentIndex}-program-${pIdx}`}>
            <Page
              size='A4'
