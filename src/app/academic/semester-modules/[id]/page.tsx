		...
		<DetailsView>
			...
			<DetailsViewBody>
				...
				<FieldView label='Module'>
					<Link size='sm' href={`/academic/modules/${item.module?.id}`}>
				...
				<FieldView label='Structure'>
					{item.semester?.structure ? (
						<Link
							size='sm'
							href={`/academic/schools/structures/${item.semester.structure.id}`}
						>
					...
					{prerequisites.length === 0 ? (
						...
						>
							{prerequisites.map((it) => (
								<ListItem key={it.id}>
									<Link size='sm' href={`/academic/semester-modules/${it.id}`}>
										{it.code} - {it.name}
									</Link>
								</ListItem>
							))}
