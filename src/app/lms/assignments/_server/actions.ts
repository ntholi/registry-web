...
export const getCourseAssignments = createAction(async (courseId: number) => {
...
export const getAssignment = createAction(
	async (courseId: number, assignmentId: number) => {
		const assignments = unwrap(await getCourseAssignments(courseId));
...
export const publishAssignment = createAction(
	async (input: {
		...
		const assignment = unwrap(
			await getAssignment(input.courseId, input.assignmentId)
		);
		if (!assignment) {
