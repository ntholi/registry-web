		...
		<Stack gap='sm'>
			<Select
				...
				onChange={(v) =>
					onUpdate({
						...question,
						responseFormat: (v as EssayQuestion['responseFormat']) || 'editor',
					})
				}
				data={[
					{ value: 'editor', label: 'HTML editor' },
					{ value: 'editorfilepicker', label: 'HTML editor with file picker' },
					{ value: 'plain', label: 'Plain text' },
					{ value: 'monospaced', label: 'Monospaced (for code)' },
					{ value: 'noinline', label: 'No inline text (attachments only)' },
				]}
			/>
			<NumberInput
			...
			<Group grow>
				<NumberInput
					...
					value={question.minWordLimit || 0}
					onChange={(v) =>
						onUpdate({ ...question, minWordLimit: Number(v) || 0 })
					}
					min={0}
				...
				<NumberInput
					...
					value={question.maxWordLimit || 0}
					onChange={(v) =>
						onUpdate({ ...question, maxWordLimit: Number(v) || 0 })
					}
					min={0}
					description='0 = no limit'
				/>
...
function NumericalEditor({
	...
	onUpdate: (q: NumericalQuestion) => void;
}) {
	const updateAnswer = (
		index: number,
		field: 'answer' | 'tolerance',
		value: string | number
	) => {
		const newAnswers = [...question.answers];
		if (field === 'answer') {
			newAnswers[index] = { ...newAnswers[index], answer: value as string };
		} else {
			newAnswers[index] = { ...newAnswers[index], tolerance: value as number };
		}
		onUpdate({ ...question, answers: newAnswers });
	};
	...
	const addAnswer = () => {
		onUpdate({
			...
			answers: [
				...question.answers,
				{ answer: '', tolerance: 0, fraction: 1, feedback: '' },
			],
		});
	};
		...
		<Stack gap='sm'>
			...
			{question.answers.map((answer, i) => (
				<Group key={answer.answer} gap='xs' align='flex-end'>
					...
					/>
					<NumberInput
						label={i === 0 ? 'Tolerance' : undefined}
						placeholder='0'
						value={answer.tolerance || 0}
						onChange={(v) => updateAnswer(i, 'tolerance', Number(v) || 0)}
						min={0}
						step={0.01}
						style={{ width: 100 }}
					/>
		...
		<Accordion.Item value={`question-${index}`} mb='md'>
			<Accordion.Control>
				<Group wrap='nowrap' align='center'>
					<ThemeIcon size='lg' radius='xl' variant='default' color='blue'>
						<Text size='sm' fw={700}>
							{index + 1}
						</Text>
					</ThemeIcon>
