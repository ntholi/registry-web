...
- Every time you finish generating code, always find lint errors and fix them when they are all fixed, run pnpm run build to make sure there are no build errors, do not stop until there are no lint or build errors. Very important! Always find a way to test the code and make sure it works as expected.
