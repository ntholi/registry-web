-- ================================================================
-- Phase 6: Preset Seeds & Cutover Readiness
-- Generated from src/app/auth/permission-presets/_lib/catalog.ts
-- Seeds permission presets and backfills users.preset_id only.
-- Route swap, proxy creation, session purge, and users.position
-- removal remain deferred until the final cutover phase.
-- ================================================================

INSERT INTO permission_presets (id, name, role, description, created_at, updated_at)
VALUES
  ('NnO3EzC5CmNgvMwYBOg4B', 'Academic Manager', 'academic', '', NOW(), NOW()),
  ('hs1hs_JgfJO5-kpoX02zu', 'Program Leader', 'academic', '', NOW(), NOW()),
  ('795-sR2cCELGEFd4B-ZZ6', 'Year Leader', 'academic', '', NOW(), NOW()),
  ('E3lzR6OXzlUS7pMHGqG6q', 'Lecturer', 'academic', '', NOW(), NOW()),
  ('_3wwvV81dhomulUdLXtTG', 'Principal Lecturer', 'academic', '', NOW(), NOW()),
  ('GkZamr-Pky9z8alJYDkNV', 'Academic Admin', 'academic', '', NOW(), NOW()),
  ('qLoHBneIKW6PhQIzpoRWY', 'Registry Staff', 'registry', '', NOW(), NOW()),
  ('9gg3zR1dQrnURcQk8SyR8', 'Registry Manager', 'registry', '', NOW(), NOW()),
  ('mDNWSEofqbLqoDLQ_hiCZ', 'Finance Staff', 'finance', '', NOW(), NOW()),
  ('E1nNl-T70le4kpF9OxLZk', 'Finance Manager', 'finance', '', NOW(), NOW()),
  ('Od2d3VxQXv8IvcPO1BFrh', 'Library Staff', 'library', '', NOW(), NOW()),
  ('6NMm7yOh66QiM9mo9oHQR', 'Marketing Staff', 'marketing', '', NOW(), NOW()),
  ('5cFJx0zqrprx5QC8yr9e9', 'Student Services Staff', 'student_services', '', NOW(), NOW()),
  ('y3c-iMGXTlOOLMnUag1gI', 'LEAP Staff', 'leap', '', NOW(), NOW()),
  ('xIZ9VravYxpKppVFxNRoJ', 'Human Resource Staff', 'human_resource', '', NOW(), NOW()),
  ('TvgQxXfhBl1AAc99L6zCf', 'Resource Staff', 'resource', '', NOW(), NOW())
ON CONFLICT (name) DO UPDATE
SET role = EXCLUDED.role,
    description = EXCLUDED.description,
    updated_at = NOW();
--> statement-breakpoint
INSERT INTO preset_permissions (id, preset_id, resource, action)
SELECT seed.id, preset.id, seed.resource, seed.action
FROM (
VALUES
  ('3Hq6TynA4R412x-SNkpVq', 'Academic Manager', 'lecturers', 'read'),
  ('bgjjyXD-ZZZk7r3xBaZxq', 'Academic Manager', 'lecturers', 'create'),
  ('s5pqYnAmpMTaNYbqzsoqK', 'Academic Manager', 'lecturers', 'update'),
  ('3QxNo9z9qGS06gx0e-H63', 'Academic Manager', 'lecturers', 'delete'),
  ('hDqkoKaEf7uleb9JNXWxT', 'Academic Manager', 'assessments', 'read'),
  ('9RtOdAyCko_Dm-CEussZh', 'Academic Manager', 'assessments', 'create'),
  ('W4D5Zu5xN9KLogsI6pna3', 'Academic Manager', 'assessments', 'update'),
  ('iimrSV1knxbMszD8da8Gh', 'Academic Manager', 'assessments', 'delete'),
  ('8_sTI-AUuDN0sV2nTXMnU', 'Academic Manager', 'student-feedback-questions', 'read'),
  ('ucvKt5wydklNvWqTahl5T', 'Academic Manager', 'student-feedback-questions', 'create'),
  ('YFKcRAdfoD949mpOohX_y', 'Academic Manager', 'student-feedback-questions', 'update'),
  ('rB1JuXtI7n6YLQ3FOSvQ3', 'Academic Manager', 'student-feedback-questions', 'delete'),
  ('ihmpuLVvFQunSz_4HaB3c', 'Academic Manager', 'student-feedback-categories', 'read'),
  ('Gh7yHgddE5S5MOIILXaSr', 'Academic Manager', 'student-feedback-categories', 'create'),
  ('KhfV5M4cgyskE7vvgXLwC', 'Academic Manager', 'student-feedback-categories', 'update'),
  ('Kcx-Ztn_z6NP5r34QiGSu', 'Academic Manager', 'student-feedback-categories', 'delete'),
  ('f7OcjbzlxeanYFoUPI9Ia', 'Academic Manager', 'feedback-cycles', 'read'),
  ('dbiKxvl59OQqFCABn4mSO', 'Academic Manager', 'feedback-cycles', 'create'),
  ('H_F6omBBj7QG7DFc829Bg', 'Academic Manager', 'feedback-cycles', 'update'),
  ('ARNftqo15uQihr11Q17xo', 'Academic Manager', 'student-feedback-reports', 'read'),
  ('kqjpErr5rFjhrCG3uNozM', 'Academic Manager', 'student-feedback-reports', 'update'),
  ('UJjgGWeqXXqkenXoKNN-0', 'Academic Manager', 'school-structures', 'read'),
  ('jWkOpwGcnkI9b0gAb1rpG', 'Academic Manager', 'school-structures', 'update'),
  ('5Yh85tcZ2XNAU0wnjbgdP', 'Academic Manager', 'school-structures', 'delete'),
  ('ma5kAqcBNm2Q8xWSdonAH', 'Academic Manager', 'timetable', 'read'),
  ('8hP54UcI-3X901ujNl3yB', 'Academic Manager', 'timetable', 'create'),
  ('X08eYdZg6howPYLP1aifh', 'Academic Manager', 'timetable', 'update'),
  ('veYiIhL9Fd6Cs0w-w0190', 'Academic Manager', 'timetable', 'delete'),
  ('tBbcGy6ZVVkkaNemZqOKt', 'Academic Manager', 'venues', 'read'),
  ('8HUMkxgsFG4AOwNWlqHDn', 'Academic Manager', 'venues', 'create'),
  ('czvgUKvjXRN_o7gxkGsQU', 'Academic Manager', 'venues', 'update'),
  ('p448OQlmMLt1e_ymy1Exw', 'Academic Manager', 'venues', 'delete'),
  ('HzBwIFIF9vqoFMcKJAlR1', 'Academic Manager', 'students', 'read'),
  ('kaQS3b-Z8vZV9M8gVtltl', 'Academic Manager', 'registration', 'read'),
  ('i-SOZTjrT3jRglT8_b8tU', 'Academic Manager', 'registration', 'update'),
    ('7wV1i0smx-SLtD7KqP2aE', 'Academic Manager', 'registration-clearance', 'read'),
  ('GxzomWrBauykwQkzc6BN-', 'Academic Manager', 'graduation', 'read'),
  ('SUZmwPKPBkQnlATXDFqTl', 'Academic Manager', 'graduation', 'approve'),
  ('WRFnd12ETjmBDELZl4w4n', 'Academic Manager', 'graduation-clearance', 'read'),
  ('5lBU1PIlNGadX_QEpQ4_0', 'Academic Manager', 'graduation-clearance', 'approve'),
  ('1NBvfrSWVunNJNA1TrsuV', 'Academic Manager', 'graduation-clearance', 'reject'),
  ('jf3TxB9hpW4gpL_kcfepR', 'Academic Manager', 'activity-tracker', 'read'),
  ('0kQclJqWyf7kY7jXXRhvq', 'Academic Manager', 'tasks', 'read'),
  ('C7uE2i50Fwm0Y1AUsJSa_', 'Academic Manager', 'tasks', 'create'),
  ('A6hBwmEuwjSN5JAtxkIst', 'Academic Manager', 'tasks', 'update'),
  ('Jjn8o1AgBGJJHsuYpAjOC', 'Academic Manager', 'tasks', 'delete'),
  ('YCA2JXKk_sBacYoH9Cbvz', 'Academic Manager', 'gradebook', 'read'),
  ('S-VAIlT4_q5F8Sc7it19x', 'Academic Manager', 'gradebook', 'update'),
  ('dWloTamRbkQ4TKT2CHesK', 'Academic Manager', 'gradebook', 'approve'),
  ('A9RxdHJYhplTsLe5cYvVJ', 'Academic Manager', 'attendance', 'read'),
  ('_y6zJ_CwiJLtmFsw4P2wM', 'Academic Manager', 'attendance', 'create'),
  ('lF6z_Rg1mm6aIdZ3xw9kf', 'Academic Manager', 'attendance', 'update'),
  ('iXUUd_-NYI-Xy2U6Tv3SG', 'Academic Manager', 'attendance', 'delete'),
  ('sDBW6WTpT3nPaZxzeXYCc', 'Academic Manager', 'assigned-modules', 'read'),
  ('n_UK_xNyc4J29Bm1gbKXi', 'Academic Manager', 'assigned-modules', 'create'),
  ('IoGNzxunpClalDfful8ox', 'Academic Manager', 'assigned-modules', 'delete'),
  ('y1rnGLiRYMlSY9WxLKDjl', 'Academic Manager', 'reports-attendance', 'read'),
  ('_BBYABGCKWpIbnFJruDhB', 'Academic Manager', 'reports-course-summary', 'read'),
  ('QzrdKgcc6plXB4otVmxNF', 'Academic Manager', 'reports-boe', 'read'),
  ('3ukR-gza3YZXctyTBfZmT', 'Academic Manager', 'reports-enrollments', 'read'),
  ('VEVtjq3QJVMj4K0OTqwFw', 'Academic Manager', 'reports-progression', 'read'),
  ('DhVGdHwQx9UG4R1JOUR4o', 'Academic Manager', 'reports-distribution', 'read'),
  ('HWrYVQokeiCJe_dwCtdBY', 'Academic Manager', 'reports-graduation', 'read'),
  ('JFsSSdd7EGAW7fSWUcuRn', 'Academic Manager', 'teaching-observations', 'read'),
  ('F-RiYsD8WLtjhZn_gSLQB', 'Academic Manager', 'teaching-observations', 'update'),
  ('sHhpM0UE4NxwzBHLQKKBb', 'Academic Manager', 'teaching-observation-criteria', 'read'),
  ('E1kTYoiBiM80m4dipHY0t', 'Academic Manager', 'teaching-observation-criteria', 'create'),
  ('RJjm-D8sxY_JfXTO2u0F5', 'Academic Manager', 'teaching-observation-criteria', 'update'),
  ('FarX5wfhMkpZeqMpetKRE', 'Academic Manager', 'teaching-observation-criteria', 'delete'),
  ('1mA1LwS5IGR0RXGXSh3WZ', 'Academic Manager', 'teaching-observation-reports', 'read'),
  ('wmFCx0K84kbjdMl2nNH5E', 'Academic Manager', 'student-statuses', 'approve'),
  ('aM1xBcDeFgHiJkLmNoPq1', 'Academic Manager', 'mails', 'read'),
  ('exmrfmHtOQEpscqcOYrMm', 'Program Leader', 'lecturers', 'read'),
  ('cCh7HzmQs9gnd2_Fgchpb', 'Program Leader', 'lecturers', 'create'),
  ('auuk3sncsMRH9IA_RtFYO', 'Program Leader', 'lecturers', 'update'),
  ('fPFNm1d3AS0-XHe61zBZm', 'Program Leader', 'lecturers', 'delete'),
  ('PPJhlxjaogMpAz8Mnu5Cy', 'Program Leader', 'student-feedback-questions', 'read'),
  ('MiCuYGQ0IzUKBYVyHBhrT', 'Program Leader', 'student-feedback-questions', 'create'),
  ('iUaK-DGghEZQHx3ozt-14', 'Program Leader', 'student-feedback-questions', 'update'),
  ('XEOP76Bvntcp--Jr7PIlJ', 'Program Leader', 'student-feedback-questions', 'delete'),
  ('6cb6W11Y4vM2AZgn_T6dV', 'Program Leader', 'student-feedback-categories', 'read'),
  ('7bWFxftBZoc-dIEjxnuiH', 'Program Leader', 'student-feedback-categories', 'create'),
  ('a1VrbUhWEJ6liiahMIqzV', 'Program Leader', 'student-feedback-categories', 'update'),
  ('LtIh_yV1Q-kFX_04dOwUF', 'Program Leader', 'student-feedback-categories', 'delete'),
  ('-Q5r5L4eqKSDgjuFtY-fl', 'Program Leader', 'school-structures', 'read'),
  ('oEuRQYnxI1t1e_RE0ncvN', 'Program Leader', 'school-structures', 'update'),
  ('iGJid0B33UYZCQtJ6gttM', 'Program Leader', 'registration', 'read'),
  ('cYxHeEdOjPPshyVzo8FKR', 'Program Leader', 'registration', 'update'),
    ('6uT0h9rLx-JKsC4NpM1bD', 'Program Leader', 'registration-clearance', 'read'),
  ('HSPDIQIWpAPUypBYwnHEi', 'Program Leader', 'timetable', 'read'),
  ('apFgOcbjRPZlCX4trUwDX', 'Program Leader', 'students', 'read'),
  ('T-J8fTOLdckMvVHSN0M1f', 'Program Leader', 'gradebook', 'read'),
  ('k-tJGQ0rvAvET3bL6wlDj', 'Program Leader', 'gradebook', 'update'),
  ('d9XWtpu5OMB36bO94pSI6', 'Program Leader', 'gradebook', 'approve'),
  ('7RbVJAv3WtSFBrfb_R59v', 'Program Leader', 'attendance', 'read'),
  ('SjIBuQXTLvggIjFjJMqfa', 'Program Leader', 'assigned-modules', 'read'),
  ('dtdoN7wbGx89pNez0JTKk', 'Program Leader', 'assigned-modules', 'create'),
  ('oRDsVliF0fUVpVZ80-pz_', 'Program Leader', 'assigned-modules', 'delete'),
  ('A2v1kJmQ9sP4dNx7Lr0Uz', 'Program Leader', 'tasks', 'read'),
  ('B3w2lKnR0tQ5eOy8Ms1Va', 'Program Leader', 'tasks', 'create'),
  ('C4x3mLoS1uR6fPz9Nt2Wb', 'Program Leader', 'tasks', 'update'),
  ('D5y4nMpT2vS7gQa0Ou3Xc', 'Program Leader', 'tasks', 'delete'),
  ('GH-_EaapOrrUgzvEmudwM', 'Program Leader', 'reports-attendance', 'read'),
  ('JQGnnIMdBWKL5WUCcHmdk', 'Program Leader', 'reports-course-summary', 'read'),
  ('-CS9RVGJlFXenyGnZ5Fj0', 'Program Leader', 'reports-boe', 'read'),
  ('qZ06gjq10guXZUAKf2ahr', 'Program Leader', 'reports-enrollments', 'read'),
  ('RdiZ-SrEZ4g4dOhz2CKP4', 'Program Leader', 'reports-progression', 'read'),
  ('LFHJ0dlDVaWzK6yXl285Q', 'Program Leader', 'reports-distribution', 'read'),
  ('Sx90UUeRUAaBFCAzFMepX', 'Program Leader', 'reports-graduation', 'read'),
  ('8G4nTJqL4n6VyJi6TWr1c', 'Program Leader', 'teaching-observations', 'read'),
  ('rDyrv0JkAGXH2r8qz9ijQ', 'Program Leader', 'teaching-observations', 'create'),
  ('x30Lj46SiRUyU7fwycqnP', 'Program Leader', 'teaching-observations', 'update'),
  ('VMwVOjNFF7u49DL0Ofgrk', 'Program Leader', 'teaching-observations', 'delete'),
  ('kk6djXyPfOAuocAAmQWN7', 'Program Leader', 'teaching-observation-criteria', 'read'),
  ('B2ftDGDtMterePwVPUF_z', 'Program Leader', 'teaching-observation-reports', 'read'),
  ('GwltkZPQC1zSsGVBHaLeQ', 'Program Leader', 'student-statuses', 'approve'),
  ('aPL2xCdEfGhIjKlMnOpQ2', 'Program Leader', 'mails', 'read'),
  ('LabAQWiB08fjiqu6YVJcY', 'Year Leader', 'feedback-cycles', 'read'),
  ('wwv-MkXvDTPkMLYsganEK', 'Year Leader', 'registration', 'read'),
    ('5tS9g8qKw-HJrB3MoN0aC', 'Year Leader', 'registration-clearance', 'read'),
  ('68Tb1cDh6YnB2RTHcCKrW', 'Year Leader', 'students', 'read'),
  ('q_TltRYcgluHV75HlJxbB', 'Year Leader', 'timetable', 'read'),
  ('ye41W__nykMO823nWGuAb', 'Year Leader', 'attendance', 'read'),
  ('E6z5oNqU3wT8hRb1Pv4Yd', 'Year Leader', 'tasks', 'read'),
  ('F7A6pOrV4xU9iSc2Qw5Ze', 'Year Leader', 'tasks', 'create'),
  ('G8B7qPsW5yV0jTd3Rx6Af', 'Year Leader', 'tasks', 'update'),
  ('H9C8rQtX6zW1kUe4Sy7Bg', 'Year Leader', 'tasks', 'delete'),
  ('5NbZiosw9aCaKk7LkjTOB', 'Year Leader', 'reports-attendance', 'read'),
  ('NeGbNVcY6RNMe0ObZPXvF', 'Year Leader', 'reports-enrollments', 'read'),
  ('qENvbtbYB9il-GUphq983', 'Year Leader', 'reports-progression', 'read'),
  ('UgiSXVxN_Ir3pz2P6KlAR', 'Year Leader', 'reports-distribution', 'read'),
  ('EoTQDwQMAEoK9md_mZ1TF', 'Year Leader', 'teaching-observations', 'read'),
  ('GDyuYbvX6apYN8HwwfxdX', 'Year Leader', 'student-statuses', 'approve'),
  ('aYL3xDdEfGhIjKlMnOpQ3', 'Year Leader', 'mails', 'read'),
  ('-qsziBlESJGnr1f0OhzK7', 'Lecturer', 'assessments', 'read'),
    ('4rP8f7nJv-GIqA2LnM9zB', 'Lecturer', 'registration', 'read'),
    ('3qN7e6mKu-FHpZ1MkL8yA', 'Lecturer', 'registration-clearance', 'read'),
  ('gLx21eB-IkAFSMcb6Em6P', 'Lecturer', 'assessments', 'create'),
  ('8g16CtaExKXkKPGxIbc47', 'Lecturer', 'assessments', 'update'),
  ('psck73CHHjvUsxwE2wO5e', 'Lecturer', 'assessments', 'delete'),
  ('oXDC6gx-JbEryJ08bZCHR', 'Lecturer', 'gradebook', 'read'),
  ('EgiAXBaBhUY9-GeCVtBPy', 'Lecturer', 'gradebook', 'update'),
  ('0D87SAIj4CyCR9v5Sk9T7', 'Lecturer', 'student-feedback-reports', 'read'),
  ('cbo73GmlSuGsfCGvAChj9', 'Lecturer', 'timetable', 'read'),
  ('lmcl80c5tNxyW40TIT17m', 'Lecturer', 'attendance', 'read'),
  ('9xU-FUvUPaXByEnv6bnG2', 'Lecturer', 'attendance', 'create'),
  ('eRwAenoNQHwE6yYwMZ2_u', 'Lecturer', 'attendance', 'update'),
  ('W4eZOpuoBqlCU7AGCPR6w', 'Lecturer', 'assigned-modules', 'read'),
  ('J1D9sRuY7AX2lVf5Tz8Ch', 'Lecturer', 'tasks', 'read'),
  ('K2E0tSvZ8BY3mWg6Ua9Di', 'Lecturer', 'tasks', 'create'),
  ('L3F1uTw09CZ4nXh7Vb0Ej', 'Lecturer', 'tasks', 'update'),
  ('M4G2vUx1AD05oYi8Wc1Fk', 'Lecturer', 'tasks', 'delete'),
  ('iTGZAxQikmX7-IArJ3Ja0', 'Lecturer', 'reports-attendance', 'read'),
  ('LL4qkSJbPfpVUn6H8o66h', 'Lecturer', 'reports-course-summary', 'read'),
  ('JCXAeb-OSIo41uT7s0MiL', 'Lecturer', 'teaching-observations', 'read'),
  ('aL4xEdFgHiJkLmNoPqRs4', 'Lecturer', 'mails', 'read'),
  ('8D-hp0nsFi9sRjaiieqv-', 'Principal Lecturer', 'assessments', 'read'),
    ('2pM6d5lJt-EGoY0LjK7xZ', 'Principal Lecturer', 'registration', 'read'),
    ('1oL5c4kIs-DFnX9KiJ6wY', 'Principal Lecturer', 'registration-clearance', 'read'),
  ('Ro18xYpi7Ja6eTwLzosY9', 'Principal Lecturer', 'assessments', 'create'),
  ('j7Kk13aBUuJ0KoUmd2Ime', 'Principal Lecturer', 'assessments', 'update'),
  ('VCk-uU-_7xFuF1ZYzdypm', 'Principal Lecturer', 'assessments', 'delete'),
  ('8UpIXeZNASQ5asQaRdPWb', 'Principal Lecturer', 'gradebook', 'read'),
  ('q0RIf7enEDzeajy7UVAAX', 'Principal Lecturer', 'gradebook', 'update'),
  ('2VSFqY0BFn6xgequCkd0m', 'Principal Lecturer', 'gradebook', 'approve'),
  ('MjaKFRk3yShbEgxWEXbRU', 'Principal Lecturer', 'student-feedback-reports', 'read'),
  ('IoisMWOd5P6siGDCwA5RD', 'Principal Lecturer', 'timetable', 'read'),
  ('0YtZ240FFutk_BtnR3zAl', 'Principal Lecturer', 'attendance', 'read'),
  ('eHNlGv4f7kxVtuasMiyHV', 'Principal Lecturer', 'attendance', 'create'),
  ('4oy0z8uy81cWCLlXGnQGK', 'Principal Lecturer', 'attendance', 'update'),
  ('cbAzqhDItl9pHpqKYz69b', 'Principal Lecturer', 'assigned-modules', 'read'),
  ('N5H3wVy2BE16pZj9Xd2Gl', 'Principal Lecturer', 'tasks', 'read'),
  ('O6I4xWz3CF27qAk0Ye3Hm', 'Principal Lecturer', 'tasks', 'create'),
  ('P7J5yXA4DG38rBl1Zf4In', 'Principal Lecturer', 'tasks', 'update'),
  ('Q8K6zYB5EH49sCm2Ag5Jo', 'Principal Lecturer', 'tasks', 'delete'),
  ('9w3CaBXRudFpPcyQW3D1J', 'Principal Lecturer', 'reports-attendance', 'read'),
  ('JDWOGibT-fSUPt6coRVAK', 'Principal Lecturer', 'reports-course-summary', 'read'),
  ('H9U5RlrxJdWV6J11pcbyL', 'Principal Lecturer', 'teaching-observations', 'read'),
  ('aP1xBcDeFgHiJkLmNoPqR', 'Principal Lecturer', 'teaching-observations', 'create'),
  ('bQ2yCdEfGhIjKlMnOpQrS', 'Principal Lecturer', 'teaching-observations', 'update'),
  ('cR3zDeEfGhIjKlMnOpQrT', 'Principal Lecturer', 'teaching-observations', 'delete'),
  ('dS4AeFfGhIjKlMnOpQrTU', 'Principal Lecturer', 'teaching-observation-criteria', 'read'),
  ('eT5BfGgHiJkLmNoPqRsUV', 'Principal Lecturer', 'users', 'read'),
  ('KAdYgmdUk2Rd8GJHZzqsu', 'Principal Lecturer', 'teaching-observation-reports', 'read'),
  ('aPrL5FgHiJkLmNoPqRsT5', 'Principal Lecturer', 'mails', 'read'),
  ('BRUswEYPXL2gq9k0tCoxf', 'Academic Admin', 'assessments', 'read'),
    ('0nK4b3jHr-CEmW8JhI5vX', 'Academic Admin', 'registration', 'read'),
    ('ZmJ3a2iGq-BDlV7IgH4uW', 'Academic Admin', 'registration-clearance', 'read'),
  ('XX87ImD-HeAw8NoWvz1Jo', 'Academic Admin', 'assessments', 'create'),
  ('jdrS87rUnAdeCNuLBk-jN', 'Academic Admin', 'assessments', 'update'),
  ('P3yrxYZX1C7gmBuw68AOO', 'Academic Admin', 'assessments', 'delete'),
  ('wmKRtkRYc-ei6PrEKdUrX', 'Academic Admin', 'student-feedback-questions', 'read'),
  ('h-mG0Mv3G2L5I-SscpQO7', 'Academic Admin', 'student-feedback-questions', 'create'),
  ('lmA6K5y6vV7HS3Yhbkb1d', 'Academic Admin', 'student-feedback-questions', 'update'),
  ('Vom5Jp5epTZ68ZHXUlBpn', 'Academic Admin', 'student-feedback-questions', 'delete'),
  ('rd0kYS9RpCe8ldpmS-oFn', 'Academic Admin', 'student-feedback-categories', 'read'),
  ('rMtcW1rwtOu8XyFikB-p6', 'Academic Admin', 'student-feedback-categories', 'create'),
  ('3-tHOItf4-_7vkE5O01Uh', 'Academic Admin', 'student-feedback-categories', 'update'),
  ('8DoKXhgZuxDOmmdm5AkGv', 'Academic Admin', 'student-feedback-categories', 'delete'),
  ('FUrdJ0PV06K4aTdD1uaOd', 'Academic Admin', 'feedback-cycles', 'read'),
  ('gUHMwQFdcynED7St4btmf', 'Academic Admin', 'feedback-cycles', 'create'),
  ('w6QRaxF1djIkjtd8uPXDE', 'Academic Admin', 'feedback-cycles', 'update'),
  ('YF4Nh4hguxGb4XN9tobg5', 'Academic Admin', 'timetable', 'read'),
  ('R9L70ZC6FI50tDn3Bh6Kp', 'Academic Admin', 'tasks', 'read'),
  ('S0M81AD7GJ61uEo4Ci7Lq', 'Academic Admin', 'tasks', 'create'),
  ('T1N92BE8HK72vFp5Dj8Mr', 'Academic Admin', 'tasks', 'update'),
  ('U2O03CF9IL83wGq6Ek9Ns', 'Academic Admin', 'tasks', 'delete'),
  ('eoF10eO2Mi9JaOMqptWM6', 'Academic Admin', 'attendance', 'read'),
  ('Kz4j8GKai7EbOkbQWq_nn', 'Academic Admin', 'teaching-observations', 'read'),
  ('ar9Qn_Xx4TMRnKb_dDt45', 'Academic Admin', 'teaching-observation-criteria', 'read'),
  ('oiEcUrCRfGq9J6QVp6PBL', 'Academic Admin', 'teaching-observation-criteria', 'create'),
  ('cMRE9YNK7-5oJxFiZ6vV5', 'Academic Admin', 'teaching-observation-criteria', 'update'),
  ('fWOf2vqlboU93Cef4GloU', 'Academic Admin', 'teaching-observation-criteria', 'delete'),
  ('3mSw7uJTwW0hPmCh7WUzE', 'Academic Admin', 'teaching-observation-reports', 'read'),
  ('aAd6GhIjKlMnOpQrStUv6', 'Academic Admin', 'mails', 'read'),
  ('VxgH_gf4DzF9aVyw2bDSs', 'Registry Staff', 'students', 'read'),
  ('gJDo2hzVPGoou7kkpvuco', 'Registry Staff', 'students', 'update'),
  ('dX63ALrQW2Rs9rASRNsfj', 'Registry Staff', 'students', 'delete'),
  ('15V36JM2PS16DNx9Lr2Uz', 'Registry Staff', 'sponsored-students', 'read'),
  ('26W47KN3QT27EOy0Ms3Va', 'Registry Staff', 'sponsored-students', 'create'),
  ('37X58LO4RU38FPz1Nt4Wb', 'Registry Staff', 'sponsored-students', 'update'),
  ('48Y69MP5SV49GQa2Ou5Xc', 'Registry Staff', 'sponsored-students', 'delete'),
  ('5ag-iH2qyBWz7Vnd-Nmtq', 'Registry Staff', 'registration', 'read'),
  ('F4f7Igxi0txmSgSRGbBnB', 'Registry Staff', 'registration', 'create'),
  ('iRWltBusSCFIn7egVXbct', 'Registry Staff', 'registration', 'update'),
  ('VBHIO6HFNCj_vacDn-0CP', 'Registry Staff', 'registration', 'delete'),
  ('VRZni4XHX1jo-e7POPpEI', 'Registry Staff', 'registration-clearance', 'read'),
  ('RgSAppRead01xA', 'Registry Staff', 'applicants', 'read'),
  ('RgSAppCreate02B', 'Registry Staff', 'applicants', 'create'),
  ('RgSAppUpdate03C', 'Registry Staff', 'applicants', 'update'),
  ('RgSAppDelete04D', 'Registry Staff', 'applicants', 'delete'),
  ('RgSAplRead05E', 'Registry Staff', 'applications', 'read'),
  ('RgSAplCreate06F', 'Registry Staff', 'applications', 'create'),
  ('RgSAplUpdate07G', 'Registry Staff', 'applications', 'update'),
  ('RgSAplDelete08H', 'Registry Staff', 'applications', 'delete'),
  ('RgSDocRead09I', 'Registry Staff', 'admissions-documents', 'read'),
  ('RgSDocCreate10J', 'Registry Staff', 'admissions-documents', 'create'),
  ('RgSDocUpdate11K', 'Registry Staff', 'admissions-documents', 'update'),
  ('RgSDocDelete12L', 'Registry Staff', 'admissions-documents', 'delete'),
  ('RgSDocApprove12M', 'Registry Staff', 'admissions-documents', 'approve'),
  ('RgSDocReject12N', 'Registry Staff', 'admissions-documents', 'reject'),
  ('RgSEntRead13M', 'Registry Staff', 'entry-requirements', 'read'),
  ('RgSEntCreate14N', 'Registry Staff', 'entry-requirements', 'create'),
  ('RgSEntUpdate15O', 'Registry Staff', 'entry-requirements', 'update'),
  ('RgSEntDelete16P', 'Registry Staff', 'entry-requirements', 'delete'),
  ('RgSSchRead17Q', 'Registry Staff', 'recognized-schools', 'read'),
  ('RgSSchCreate18R', 'Registry Staff', 'recognized-schools', 'create'),
  ('RgSSchUpdate19S', 'Registry Staff', 'recognized-schools', 'update'),
  ('RgSSchDelete20T', 'Registry Staff', 'recognized-schools', 'delete'),
  ('RgSIntRead21U', 'Registry Staff', 'intake-periods', 'read'),
  ('RgSIntCreate22V', 'Registry Staff', 'intake-periods', 'create'),
  ('RgSIntUpdate23W', 'Registry Staff', 'intake-periods', 'update'),
  ('RgSIntDelete24X', 'Registry Staff', 'intake-periods', 'delete'),
  ('RgSCertRead25Y', 'Registry Staff', 'certificate-types', 'read'),
  ('RgSCertCreate26Z', 'Registry Staff', 'certificate-types', 'create'),
  ('RgSCertUpdate27a', 'Registry Staff', 'certificate-types', 'update'),
  ('RgSCertDelete28b', 'Registry Staff', 'certificate-types', 'delete'),
  ('RgSSubRead29c', 'Registry Staff', 'subjects', 'read'),
  ('RgSSubCreate30d', 'Registry Staff', 'subjects', 'create'),
  ('RgSSubUpdate31e', 'Registry Staff', 'subjects', 'update'),
  ('RgSSubDelete32f', 'Registry Staff', 'subjects', 'delete'),
  ('cR0eif_qtRKYOic1IMJzA', 'Registry Staff', 'documents', 'read'),
  ('B9H2WI4gWeVl0uckhXYDS', 'Registry Staff', 'documents', 'create'),
  ('sNa0FJOnqznrrtxfauSdM', 'Registry Staff', 'student-statuses', 'read'),
  ('6PXodWykk2S-BIycBnKVo', 'Registry Staff', 'student-statuses', 'create'),
  ('dtsCYnQg9qo-T-ceKKpwP', 'Registry Staff', 'student-statuses', 'update'),
  ('4inAr2_lpEoYPYuNR013L', 'Registry Staff', 'student-statuses', 'delete'),
  ('aNNabaCOJUakgpzdcD948', 'Registry Staff', 'terms-settings', 'read'),
  ('gJFCncadgqVQKXI7xH36g', 'Registry Staff', 'terms-settings', 'update'),
  ('4qld05bsjMxBcHUoGOUZq', 'Registry Staff', 'certificate-reprints', 'read'),
  ('ap_hKqmvlmiDqn68OgB13', 'Registry Staff', 'certificate-reprints', 'create'),
  ('JuXoLiNJdNZKzzEtEZG3C', 'Registry Staff', 'certificate-reprints', 'update'),
  ('dFvDSTJe7OX8IW0jCID26', 'Registry Staff', 'certificate-reprints', 'delete'),
  ('LYYbHxpda0ORL7kJXYJr2', 'Registry Staff', 'modules', 'create'),
  ('MisiioRNw0WiELkPrAQTy', 'Registry Staff', 'semester-modules', 'create'),
  ('j7C8ZGZVf_UQcF87wvNR_', 'Registry Staff', 'semester-modules', 'update'),
  ('Rgi_auMXW1HceBrV7MZgl', 'Registry Staff', 'venues', 'create'),
  ('khKeprjtuG-z_zRHQp8_n', 'Registry Staff', 'venues', 'update'),
  ('gWW12BBLaDkw2b3ZdwOjh', 'Registry Staff', 'venues', 'delete'),
  ('rysyMdetLCXlevS60ks9S', 'Registry Staff', 'graduation', 'read'),
  ('V3P14DG0JM94xHr7Fl0Ot', 'Registry Staff', 'tasks', 'read'),
  ('W4Q25EH1KN05yIs8Gm1Pu', 'Registry Staff', 'tasks', 'create'),
  ('X5R36FI2LO16zJt9Hn2Qv', 'Registry Staff', 'tasks', 'update'),
  ('Y6S47GJ3MP27AKu0Io3Rw', 'Registry Staff', 'tasks', 'delete'),
  ('ErqBvxoyGzmmSLv5bZbA6', 'Registry Staff', 'student-notes', 'read'),
  ('-wjVWdbOJB0h-H0_T3UGT', 'Registry Staff', 'student-notes', 'create'),
  ('I8DDJLtOZPxvkP_IvkBXk', 'Registry Staff', 'student-notes', 'update'),
  ('fbDEIW6Jgj9LVQuFzuxVl', 'Registry Staff', 'student-notes', 'delete'),
  ('YU6UYmXJ0BDT0Gr_dUgie', 'Registry Staff', 'blocked-students', 'read'),
  ('VqWSzJEpDfJZPEvPQqvTM', 'Registry Staff', 'blocked-students', 'create'),
  ('3LjeJqd06FWIVNedVWylk', 'Registry Staff', 'blocked-students', 'update'),
  ('jVAN191ANhsjwlf1Hy5NL', 'Registry Staff', 'reports-enrollments', 'read'),
  ('3eEYqxX4IQuIc9u8pql_L', 'Registry Staff', 'reports-progression', 'read'),
  ('IawcfS9De_E0ur7f3TCi6', 'Registry Staff', 'reports-distribution', 'read'),
  ('a6tEGLuNrCT8G-2ZryE8-', 'Registry Staff', 'reports-attendance', 'read'),
  ('rS7HiJkLmNoPqRsTuVw7a', 'Registry Staff', 'mails', 'read'),
  ('n62C2vmGQLlwwK17BI7hf', 'Registry Manager', 'students', 'read'),
  ('O62v3Db2Mt4OFQw3a8Dva', 'Registry Manager', 'students', 'update'),
  ('qpLAwoAiL13IDrJVIlNaD', 'Registry Manager', 'students', 'delete'),
  ('59Z70NQ6TW50HRb3Pv6Yd', 'Registry Manager', 'sponsors', 'read'),
  ('6aA81OR7UX61ISc4Qw7Ze', 'Registry Manager', 'sponsors', 'create'),
  ('7bB92PS8VY72JTd5Rx8Af', 'Registry Manager', 'sponsors', 'update'),
  ('8cC03QT9WZ83KUe6Sy9Bg', 'Registry Manager', 'sponsors', 'delete'),
  ('9dD14RU0XA94LVe7Tz0Ch', 'Registry Manager', 'sponsored-students', 'read'),
  ('AeE25SV1YB05MWf8Ua1Di', 'Registry Manager', 'sponsored-students', 'create'),
  ('BfF36TW2ZC16NXg9Vb2Ej', 'Registry Manager', 'sponsored-students', 'update'),
  ('CgG47UX3AD27OYh0Wc3Fk', 'Registry Manager', 'sponsored-students', 'delete'),
  ('8-YUm3_gzMGY0QxIV9jTE', 'Registry Manager', 'registration', 'read'),
  ('7G1TfaBDei4PCTzhC4cIo', 'Registry Manager', 'registration', 'create'),
  ('O_bltTGi-luMp9btFZXqv', 'Registry Manager', 'registration', 'update'),
  ('VT9GUmQS_T2TbLCX9Qo5Y', 'Registry Manager', 'registration', 'delete'),
  ('5Y1P6SYQiCgRmaPTuVtNk', 'Registry Manager', 'registration-clearance', 'read'),
  ('RgMAppRead33g', 'Registry Manager', 'applicants', 'read'),
  ('RgMAppCreate34h', 'Registry Manager', 'applicants', 'create'),
  ('RgMAppUpdate35i', 'Registry Manager', 'applicants', 'update'),
  ('RgMAppDelete36j', 'Registry Manager', 'applicants', 'delete'),
  ('RgMAplRead37k', 'Registry Manager', 'applications', 'read'),
  ('RgMAplCreate38l', 'Registry Manager', 'applications', 'create'),
  ('RgMAplUpdate39m', 'Registry Manager', 'applications', 'update'),
  ('RgMAplDelete40n', 'Registry Manager', 'applications', 'delete'),
  ('RgMDocRead41o', 'Registry Manager', 'admissions-documents', 'read'),
  ('RgMDocCreate42p', 'Registry Manager', 'admissions-documents', 'create'),
  ('RgMDocUpdate43q', 'Registry Manager', 'admissions-documents', 'update'),
  ('RgMDocDelete44r', 'Registry Manager', 'admissions-documents', 'delete'),
  ('RgMDocApprove44s', 'Registry Manager', 'admissions-documents', 'approve'),
  ('RgMDocReject44t', 'Registry Manager', 'admissions-documents', 'reject'),
  ('RgMEntRead45s', 'Registry Manager', 'entry-requirements', 'read'),
  ('RgMEntCreate46t', 'Registry Manager', 'entry-requirements', 'create'),
  ('RgMEntUpdate47u', 'Registry Manager', 'entry-requirements', 'update'),
  ('RgMEntDelete48v', 'Registry Manager', 'entry-requirements', 'delete'),
  ('RgMSchRead49w', 'Registry Manager', 'recognized-schools', 'read'),
  ('RgMSchCreate50x', 'Registry Manager', 'recognized-schools', 'create'),
  ('RgMSchUpdate51y', 'Registry Manager', 'recognized-schools', 'update'),
  ('RgMSchDelete52z', 'Registry Manager', 'recognized-schools', 'delete'),
  ('RgMIntRead53A', 'Registry Manager', 'intake-periods', 'read'),
  ('RgMIntCreate54B', 'Registry Manager', 'intake-periods', 'create'),
  ('RgMIntUpdate55C', 'Registry Manager', 'intake-periods', 'update'),
  ('RgMIntDelete56D', 'Registry Manager', 'intake-periods', 'delete'),
  ('RgMCertRead57E', 'Registry Manager', 'certificate-types', 'read'),
  ('RgMCertCreate58F', 'Registry Manager', 'certificate-types', 'create'),
  ('RgMCertUpdate59G', 'Registry Manager', 'certificate-types', 'update'),
  ('RgMCertDelete60H', 'Registry Manager', 'certificate-types', 'delete'),
  ('RgMSubRead61I', 'Registry Manager', 'subjects', 'read'),
  ('RgMSubCreate62J', 'Registry Manager', 'subjects', 'create'),
  ('RgMSubUpdate63K', 'Registry Manager', 'subjects', 'update'),
  ('RgMSubDelete64L', 'Registry Manager', 'subjects', 'delete'),
  ('n5oIL4dC4fPHVQtt2V9NL', 'Registry Manager', 'documents', 'read'),
  ('a2cAFhfcs2BXSmn79UOGA', 'Registry Manager', 'documents', 'create'),
  ('4BJA6v3dqR9KrOg_3gjEV', 'Registry Manager', 'student-statuses', 'read'),
  ('NpYoxVBi_0BDK961nZyqR', 'Registry Manager', 'student-statuses', 'create'),
  ('4OxmumWRVzz1pTupUfz_5', 'Registry Manager', 'student-statuses', 'update'),
  ('QNfCAsv2y3twc0wJiyCjw', 'Registry Manager', 'student-statuses', 'delete'),
  ('nW9cPjmvqM_qb72FaM6hW', 'Registry Manager', 'terms-settings', 'read'),
  ('4YiSdw-LQ9FpW4dMJ2gf_', 'Registry Manager', 'terms-settings', 'update'),
  ('44LZ9VGnnnzhk06NhJ1o_', 'Registry Manager', 'certificate-reprints', 'read'),
  ('CtrVJHhZ7hHsroHsSqu1w', 'Registry Manager', 'certificate-reprints', 'create'),
  ('lOgJvvJjuG78Iv6tZcV19', 'Registry Manager', 'certificate-reprints', 'update'),
  ('V7Oh9Kh5W847Btvdy4W0_', 'Registry Manager', 'certificate-reprints', 'delete'),
  ('yFJvGYQvfxYXORFDOx8sn', 'Registry Manager', 'modules', 'create'),
  ('ajGEUA41aY3Pqx79DAA-B', 'Registry Manager', 'semester-modules', 'create'),
  ('kOkxwZOTFIhJTff3Hejsq', 'Registry Manager', 'semester-modules', 'update'),
  ('mybLCTqyHkw79ByhEzgfT', 'Registry Manager', 'venues', 'create'),
  ('oC5yLaxY3k3ZBMFhlmWsx', 'Registry Manager', 'venues', 'update'),
  ('65RVEm8zfT1uHYdOsIbh5', 'Registry Manager', 'venues', 'delete'),
  ('AU3bPomnHFl7BOyP_6WDp', 'Registry Manager', 'graduation', 'read'),
  ('vq1aVFAPP2N0rcmqpBsJx', 'Registry Manager', 'activity-tracker', 'read'),
  ('8oRdYJl5RO3cvD5UejHAR', 'Registry Manager', 'tasks', 'read'),
  ('ZpB_TxGXqV1NqiuowT4c-', 'Registry Manager', 'tasks', 'create'),
  ('YHCIPVJPyizqzXswUdBhd', 'Registry Manager', 'tasks', 'update'),
  ('tqG6XTtgztu2aeeYECUpv', 'Registry Manager', 'tasks', 'delete'),
  ('G3NLQMQpwDuRFIPi3aYwt', 'Registry Manager', 'school-structures', 'update'),
  ('56WATBfjQ33Cbzv6wygl1', 'Registry Manager', 'student-notes', 'read'),
  ('-YtG8YbkyTdBaI3cCQiMh', 'Registry Manager', 'student-notes', 'create'),
  ('vPdPdf-aAmErIISFQGpuV', 'Registry Manager', 'student-notes', 'update'),
  ('2NvcJBO2v_CqQ6DgBmBTL', 'Registry Manager', 'student-notes', 'delete'),
  ('3PBnDC17E5AHk3gLpRKJa', 'Registry Manager', 'blocked-students', 'read'),
  ('ZqFQmVHju20FKEYWUYDfs', 'Registry Manager', 'blocked-students', 'create'),
  ('XQhSqku6ootMU4MGI_nhG', 'Registry Manager', 'blocked-students', 'update'),
  ('PwIeW83Zdvn9pJtMS1h0f', 'Registry Manager', 'notifications', 'read'),
  ('mRg8xKp3vQ2nLwYhJ5fZa', 'Registry Manager', 'mails', 'read'),
  ('q3kYQYsJjlN7FFxmWOP-Q', 'Registry Manager', 'reports-enrollments', 'read'),
  ('3CpOHPeYW0RA1fsRtPsHL', 'Registry Manager', 'reports-progression', 'read'),
  ('WSsUXfTeMkBA8H-rNQzYh', 'Registry Manager', 'reports-distribution', 'read'),
  ('B2Ix99GwvJQL89NOqXBa9', 'Registry Manager', 'reports-attendance', 'read'),
  ('_5SumG8iAl9y40Cv7mIzz', 'Registry Manager', 'reports-graduation', 'read'),
  ('wYf_CdfZB2JBaLwBmJVhB', 'Finance Staff', 'sponsors', 'read'),
  ('ndHGgEO_JXwua8NbaBXf9', 'Finance Staff', 'sponsors', 'create'),
  ('5boxT254nlpIu6YQIvs5e', 'Finance Staff', 'sponsors', 'update'),
  ('3_19Z1kbqIFpYIvVC62jp', 'Finance Staff', 'sponsors', 'delete'),
  ('DhH58VY4BE38PZi1Xd4Gl', 'Finance Staff', 'sponsored-students', 'read'),
  ('EiI69WZ5CF49QAj2Ye5Hm', 'Finance Staff', 'sponsored-students', 'create'),
  ('FjJ70XA6DG50RBk3Zf6In', 'Finance Staff', 'sponsored-students', 'update'),
  ('GkK81YB7EH61SCl4Ag7Jo', 'Finance Staff', 'sponsored-students', 'delete'),
  ('LpP36DG2JM16XHq9Fl2Ot', 'Finance Staff', 'zoho', 'read'),
  ('MqQ47EH3KN27YIr0Gm3Pu', 'Finance Staff', 'zoho', 'create'),
  ('NrR58FI4LO38ZJs1Hn4Qv', 'Finance Staff', 'zoho', 'update'),
  ('OsS69GJ5MP49AKt2Io5Rw', 'Finance Staff', 'zoho', 'delete'),
  ('38xpCM0ckmM-xzVY6h1jj', 'Finance Staff', 'admissions-payments', 'read'),
  ('O1wfnUg_AbUkL7LqKVdHF', 'Finance Staff', 'admissions-payments', 'update'),
  ('HEMTeHEZxW1FREdJXp00U', 'Finance Staff', 'student-statuses', 'read'),
  ('DsUX_KL_qfLFZdhwQo_by', 'Finance Staff', 'graduation', 'read'),
  ('0Z6T1NOuQYYIJtF3MWA5Z', 'Finance Staff', 'students', 'read'),
    ('YlI2z1hFp-ACkU6HfG3tV', 'Finance Staff', 'registration', 'read'),
  ('21DBhDgLc0NSytKOzWtJd', 'Finance Staff', 'registration-clearance', 'read'),
  ('illbi-PI3Cckm3V98hNJB', 'Finance Staff', 'registration-clearance', 'approve'),
  ('PdEhJ5aIivceIfaszHl9y', 'Finance Staff', 'registration-clearance', 'reject'),
  ('iZsBmzvukPuhv3p8ECudN', 'Finance Staff', 'graduation-clearance', 'read'),
  ('_Szirv-HTbchL8f1NgU_Q', 'Finance Staff', 'graduation-clearance', 'approve'),
  ('jAzA2dfXtnnPRJyveqrLi', 'Finance Staff', 'graduation-clearance', 'reject'),
  ('BCV01lK-nX_EvXcMeeGfR', 'Finance Staff', 'blocked-students', 'read'),
  ('1mmmHW5qLXC5uyGhB1_KD', 'Finance Staff', 'blocked-students', 'create'),
  ('Xv-jY9c7fnWuPJS-fD0W5', 'Finance Staff', 'blocked-students', 'update'),
  ('Z7T58HK4NQ38BLv1Jp4Sx', 'Finance Staff', 'tasks', 'read'),
  ('a8U69IL5OR49CMw2Kq5Ty', 'Finance Staff', 'tasks', 'create'),
  ('b9V70JM6PS50DNx3Lr6Uz', 'Finance Staff', 'tasks', 'update'),
  ('c0W81KN7QT61EOy4Ms7Va', 'Finance Staff', 'tasks', 'delete'),
  ('U4uJ9Z-GX1Re3rTZwJwsR', 'Finance Staff', 'reports-enrollments', 'read'),
  ('lcowiFYhH3DL6DyDYgCx8', 'Finance Staff', 'reports-progression', 'read'),
  ('CO4hA8n3kGl1bBiS0JKry', 'Finance Staff', 'reports-distribution', 'read'),
  ('eHBiNInopGXbWr_nHv7lz', 'Finance Staff', 'reports-graduation', 'read'),
  ('B5u7SH6XfdIjXKYqYicBc', 'Finance Staff', 'reports-sponsored-students', 'read'),
  ('xUknjW7fxpXLQ1E4B-sQu', 'Finance Staff', 'student-statuses', 'approve'),
  ('fS8IjKlMnOpQrStUvWx8a', 'Finance Staff', 'mails', 'read'),
  ('_ndq0J4YQRtzAgWgDCSVY', 'Finance Manager', 'sponsors', 'read'),
  ('mstI-T8GYTrChzljnRqqU', 'Finance Manager', 'sponsors', 'create'),
  ('khI7DLfyCALkRTEpfRzF3', 'Finance Manager', 'sponsors', 'update'),
  ('4tPZf1nsL-2aLtjW_SB7m', 'Finance Manager', 'sponsors', 'delete'),
  ('HlL92ZC8FI72TDm5Bh8Kp', 'Finance Manager', 'sponsored-students', 'read'),
  ('ImM03AD9GJ83UEn6Ci9Lq', 'Finance Manager', 'sponsored-students', 'create'),
  ('JnN14BE0HK94VFo7Dj0Mr', 'Finance Manager', 'sponsored-students', 'update'),
  ('KoO25CF1IL05WGp8Ek1Ns', 'Finance Manager', 'sponsored-students', 'delete'),
  ('PtT70HK6NQ50BLu3Jp6Sx', 'Finance Manager', 'zoho', 'read'),
  ('QuU81IL7OR61CMv4Kq7Ty', 'Finance Manager', 'zoho', 'create'),
  ('RvV92JM8PS72DNw5Lr8Uz', 'Finance Manager', 'zoho', 'update'),
  ('SwW03KN9QT83EOx6Ms9Va', 'Finance Manager', 'zoho', 'delete'),
  ('lif4vT-fXPaWlA6EC1boE', 'Finance Manager', 'admissions-payments', 'read'),
  ('hQaM8pT4lp3GrAcRWSIog', 'Finance Manager', 'admissions-payments', 'update'),
  ('m3PR7T7fxIk7M1qLt15Uq', 'Finance Manager', 'student-statuses', 'read'),
  ('AdMNyamyn40le4Q8oVJHO', 'Finance Manager', 'graduation', 'read'),
  ('SEiA1qenT0mmHRfkz-rAS', 'Finance Manager', 'students', 'read'),
    ('XkH1y0gEo-ZBjT5GeF2sU', 'Finance Manager', 'registration', 'read'),
  ('5WGEE9kQJOyOZf0saWyRm', 'Finance Manager', 'registration-clearance', 'read'),
  ('67eM433FzsjljSCIgIcyL', 'Finance Manager', 'registration-clearance', 'approve'),
  ('d7etQyNcGapfMohIfoCbI', 'Finance Manager', 'registration-clearance', 'reject'),
  ('t_rWlOdco7ARj_kp--x3N', 'Finance Manager', 'graduation-clearance', 'read'),
  ('f__r6jIQ-H-a0U__Obipr', 'Finance Manager', 'graduation-clearance', 'approve'),
  ('sFvCS1JVyAqGdVys_DeMa', 'Finance Manager', 'graduation-clearance', 'reject'),
  ('-N0p9MUzDc9omrjFw-L65', 'Finance Manager', 'blocked-students', 'read'),
  ('R36R4_3hkfqtLWlc7rGBE', 'Finance Manager', 'blocked-students', 'create'),
  ('IGJifwFJnJmFsGXUwV6PX', 'Finance Manager', 'blocked-students', 'update'),
  ('vA7nXq2mS8rT5uY1cD4eF', 'Finance Staff', 'auto-approvals', 'read'),
  ('wB8oYr3nT9sU6vZ2dE5fG', 'Finance Staff', 'auto-approvals', 'create'),
  ('xC9pZs4oU0tV7wA3eF6gH', 'Finance Staff', 'auto-approvals', 'update'),
  ('yD0qAt5pV1uW8xB4fG7hI', 'Finance Staff', 'auto-approvals', 'delete'),
  ('FlQcI9EsuqU6x1TJr2Die', 'Finance Manager', 'auto-approvals', 'read'),
  ('jRekX5Z0YsYvFXMFApA6h', 'Finance Manager', 'auto-approvals', 'create'),
  ('bVr5p3YfCVqHXFizJdHUn', 'Finance Manager', 'auto-approvals', 'update'),
  ('wyXeoqmCWs4RQTzWE5BcU', 'Finance Manager', 'auto-approvals', 'delete'),
  ('ikw9UpL9X1-gP-y2JXYIo', 'Finance Manager', 'activity-tracker', 'read'),
  ('M39v87hHF3aHIAa7YmC9x', 'Finance Manager', 'tasks', 'read'),
  ('KYcMPwQW0YWFerAEiLDg8', 'Finance Manager', 'tasks', 'create'),
  ('xuPTzEk70--i7CpEyS3tv', 'Finance Manager', 'tasks', 'update'),
  ('7LcMYqAwSOFdejxRKegvb', 'Finance Manager', 'tasks', 'delete'),
  ('8iicXXFCN5-Tk0GaoANnc', 'Finance Manager', 'reports-enrollments', 'read'),
  ('VFYWR8CFcLYV7Zj5oqo4J', 'Finance Manager', 'reports-progression', 'read'),
  ('55121-SxxnKhyDWC3rs5V', 'Finance Manager', 'reports-distribution', 'read'),
  ('oWTARXZUMgoXKR3pqQBp3', 'Finance Manager', 'reports-graduation', 'read'),
  ('ZxyfIGZFDx2DOM_V5jBlW', 'Finance Manager', 'reports-sponsored-students', 'read'),
  ('EL-Q-yiSMsBIda5eOXvWu', 'Finance Manager', 'student-statuses', 'approve'),
  ('fM9JkLmNoPqRsTuVwXy9a', 'Finance Manager', 'mails', 'read'),
  ('x-loM66REgljh63soNr-o', 'Library Staff', 'library', 'read'),
  ('UIFOSm7m78au5aKejZfF4', 'Library Staff', 'library', 'create'),
  ('se1lvH7XHMVY90nmlEeWe', 'Library Staff', 'library', 'update'),
  ('j4bM1mZ5VhDiCdrDYpxTX', 'Library Staff', 'library', 'delete'),
  ('qAzme47M7nIFMIhRNFWkL', 'Library Staff', 'students', 'read'),
    ('WjG0x9fDn-YAiS4FdE1rT', 'Library Staff', 'registration', 'read'),
  ('DXGWN0GgtBmV094K9qqTU', 'Library Staff', 'registration-clearance', 'read'),
  ('NIeukvANTM228I9Bcw71q', 'Library Staff', 'registration-clearance', 'approve'),
  ('4jN3c-0fhK-5i1mhr-xkp', 'Library Staff', 'registration-clearance', 'reject'),
  ('-JrMp79giav06Uo_vR_x6', 'Library Staff', 'graduation-clearance', 'read'),
  ('3czTnAqkq2PiTBDaVR735', 'Library Staff', 'graduation-clearance', 'approve'),
  ('r-QZTbtMGOrZaKOdvqNcK', 'Library Staff', 'graduation-clearance', 'reject'),
  ('d1X92LO8RU72FPz5Nt8Wb', 'Library Staff', 'tasks', 'read'),
  ('e2Y03MP9SV83GQa6Ou9Xc', 'Library Staff', 'tasks', 'create'),
  ('f3Z14NQ0TW94HRb7Pv0Yd', 'Library Staff', 'tasks', 'update'),
  ('g4A25OR1UX05ISc8Qw1Ze', 'Library Staff', 'tasks', 'delete'),
  ('7mXXEC9f_GTiAEgJLTlpK', 'Library Staff', 'auto-approvals', 'read'),
  ('ejQssK3lfanLEJo1zKNnI', 'Library Staff', 'auto-approvals', 'create'),
  ('sFLPMBkv6FpK688Rz9t-u', 'Library Staff', 'auto-approvals', 'update'),
  ('NuB9mX_oVRCccUuC9m9pb', 'Library Staff', 'auto-approvals', 'delete'),
  ('lS0KlMnOpQrStUvWxYz0a', 'Library Staff', 'mails', 'read'),
  ('Xvko0glA4lbmLruYmYp0R', 'Marketing Staff', 'applicants', 'read'),
  ('hHxc1I7fWsbD9VutoHBvd', 'Marketing Staff', 'applicants', 'create'),
  ('Qi4FsJmKczC4qSM2jBJ-M', 'Marketing Staff', 'applicants', 'update'),
  ('4GdzMGbQAGdvDX465QAVe', 'Marketing Staff', 'applicants', 'delete'),
  ('1EpJmEUHXR6g0TUEOjCof', 'Marketing Staff', 'applications', 'read'),
  ('p_0tKlV09jrMZAy6nN3n5', 'Marketing Staff', 'applications', 'create'),
  ('Q1j99xI7KTzgrupdCjb1O', 'Marketing Staff', 'applications', 'update'),
  ('qpqncjEcCKrlJ3E7IGKam', 'Marketing Staff', 'applications', 'delete'),
  ('RhZrO80A56tebTDp0iejW', 'Marketing Staff', 'entry-requirements', 'read'),
  ('B7-T2c5nHj3L0POQpq50m', 'Marketing Staff', 'entry-requirements', 'create'),
  ('RbQS9iUctJBe1NunHPnWY', 'Marketing Staff', 'entry-requirements', 'update'),
  ('vjFlV39wdX6K5qp3N1JQc', 'Marketing Staff', 'entry-requirements', 'delete'),
  ('fwDFNC8MrPvEhzjq-k3-W', 'Marketing Staff', 'admissions-payments', 'read'),
  ('qUq1591sLkyUaxPk0HuWK', 'Marketing Staff', 'admissions-documents', 'read'),
  ('MktDocUpd65m', 'Marketing Staff', 'admissions-documents', 'update'),
  ('MktDocAppr66n', 'Marketing Staff', 'admissions-documents', 'approve'),
  ('MktDocRej67o', 'Marketing Staff', 'admissions-documents', 'reject'),
  ('TpMGD4UKF_Wr_sTCqmL2i', 'Marketing Staff', 'recognized-schools', 'read'),
  ('3VM5tmARWj2ohCXgRi-HV', 'Marketing Staff', 'recognized-schools', 'create'),
  ('OTpQ0Z7krCJl9PHnnx5zX', 'Marketing Staff', 'recognized-schools', 'update'),
  ('Eciyw4OimQ7ZIP4RVwWGf', 'Marketing Staff', 'recognized-schools', 'delete'),
  ('9jvRLFKvuTDgLLzwZdHkO', 'Marketing Staff', 'intake-periods', 'read'),
  ('WWWwfhQbOuwmyeLSmhUKm', 'Marketing Staff', 'intake-periods', 'create'),
  ('u1tmWfajXMN_2bXECt-C5', 'Marketing Staff', 'intake-periods', 'update'),
  ('Cn3YJbEXEm14w33Iqt1f4', 'Marketing Staff', 'intake-periods', 'delete'),
  ('cwzcaAelzkk7CobphqkBk', 'Marketing Staff', 'certificate-types', 'read'),
  ('50jgu1C019i6L4QWPMu9P', 'Marketing Staff', 'certificate-types', 'create'),
  ('-aAHNyz-chzpNUJXqSoAA', 'Marketing Staff', 'certificate-types', 'update'),
  ('uvytwX1ShFkKtEvgFZel_', 'Marketing Staff', 'certificate-types', 'delete'),
  ('h5B36PS2VY16JTd9Rx2Af', 'Marketing Staff', 'tasks', 'read'),
  ('i6C47QT3WZ27KUe0Sy3Bg', 'Marketing Staff', 'tasks', 'create'),
  ('j7D58RU4XA38LVf1Tz4Ch', 'Marketing Staff', 'tasks', 'update'),
  ('k8E69SV5YB49MWg2Ua5Di', 'Marketing Staff', 'tasks', 'delete'),
  ('Gg4PAnMvattBP0DnegAmM', 'Marketing Staff', 'subjects', 'read'),
  ('gTxzHD2uJoeRtaRomrPiv', 'Marketing Staff', 'subjects', 'create'),
  ('4mzsYNPyJPPrCAdwFh_H-', 'Marketing Staff', 'subjects', 'update'),
  ('J3M3YAUckbxeMUXyRaMBL', 'Marketing Staff', 'subjects', 'delete'),
  ('mS1LmNoPqRsTuVwXyZa1b', 'Marketing Staff', 'mails', 'read'),
  ('amEpGbqNNmtay-ZdMu55u', 'Student Services Staff', 'students', 'read'),
  ('_-Ku3pSPwKNNacvRvMKE9', 'Student Services Staff', 'students', 'update'),
  ('Dm4M4auF4MI8SJOOdIBDm', 'Student Services Staff', 'registration', 'read'),
  ('q66tolEu0pQRDxN1jcxcg', 'Student Services Staff', 'registration', 'update'),
    ('ViF9w8eCm-XZhR3EcD0qS', 'Student Services Staff', 'registration-clearance', 'read'),
  ('upGX6BgyIppH_Sgz19LKZ', 'Student Services Staff', 'documents', 'read'),
  ('qLC6PDzKJ1HQ0Twf7Jv1m', 'Student Services Staff', 'documents', 'create'),
  ('pA8b3_4yNAAKm45Cyzfh1', 'Student Services Staff', 'student-statuses', 'read'),
  ('Q4hPHTGo7flsyupSs3UKH', 'Student Services Staff', 'student-statuses', 'create'),
  ('HXxUQnNainadNxjn7cB_l', 'Student Services Staff', 'student-statuses', 'update'),
  ('l9F70TW6ZC50NXh3Vb6Ej', 'Student Services Staff', 'tasks', 'read'),
  ('m0G81UX7AD61OYi4Wc7Fk', 'Student Services Staff', 'tasks', 'create'),
  ('n1H92VY8BE72PZj5Xd8Gl', 'Student Services Staff', 'tasks', 'update'),
  ('o2I03WZ9CF83QAk6Ye9Hm', 'Student Services Staff', 'tasks', 'delete'),
  ('2HHJCQp5OY6hJSL8dMI3L', 'Student Services Staff', 'student-notes', 'read'),
  ('UwZMYs0ZqTYRtRYE1_3e8', 'Student Services Staff', 'student-notes', 'create'),
  ('KuBm63MC5rGV32z9DPzBj', 'Student Services Staff', 'student-notes', 'update'),
  ('rLl45XOxuy2FVhi7Gzc1M', 'Student Services Staff', 'student-notes', 'delete'),
  ('cd-mikDYHFVgDn56Kgj2i', 'Student Services Staff', 'student-statuses', 'approve'),
  ('sS2MnOpQrStUvWxYzAb2c', 'Student Services Staff', 'mails', 'read'),
  ('QMu16eCalvBOw2uZGIir-', 'LEAP Staff', 'assessments', 'read'),
  ('87EqwW6FSi04hPaBpTW4f', 'LEAP Staff', 'assessments', 'create'),
  ('R37tgQAqZ-2lpxlnTN1ss', 'LEAP Staff', 'assessments', 'update'),
  ('yixMY66oPyp5122BTQ5Ef', 'LEAP Staff', 'assessments', 'delete'),
  ('ONWpHYsuNywgOzqFXEM9S', 'LEAP Staff', 'registration', 'read'),
  ('a3s2tzNipWyZhaTv4E-dg', 'LEAP Staff', 'registration', 'update'),
  ('Ptrhtt9lV2lfEDCrn-glY', 'LEAP Staff', 'registration-clearance', 'read'),
  ('IRdxPAMJYlYDKQwFecPh4', 'LEAP Staff', 'students', 'read'),
  ('1rbGp-btAT-Advf2M8l8I', 'LEAP Staff', 'attendance', 'read'),
  ('DmZCK0XigNXV4e0HRcJC6', 'LEAP Staff', 'attendance', 'create'),
  ('sVwuCK3WErGexnxCDKRBd', 'LEAP Staff', 'attendance', 'update'),
  ('arnmPqrhEC8BAL1vFWVRP', 'LEAP Staff', 'assigned-modules', 'read'),
  ('p3J14XA0DG94RBl7Zf0In', 'LEAP Staff', 'tasks', 'read'),
  ('q4K25YB1EH05SCm8Ag1Jo', 'LEAP Staff', 'tasks', 'create'),
  ('r5L36ZC2FI16TDn9Bh2Kp', 'LEAP Staff', 'tasks', 'update'),
  ('s6M47AD3GJ27UEo0Ci3Lq', 'LEAP Staff', 'tasks', 'delete'),
  ('g-om_SlvbiDMBittf143z', 'LEAP Staff', 'reports-attendance', 'read'),
  ('L3m6OCMlzaK_q9KjqX1TY', 'LEAP Staff', 'reports-enrollments', 'read'),
  ('UVbBUIthBksfSWE5ZEuAP', 'LEAP Staff', 'reports-progression', 'read'),
  ('afwaJ0bWn3RL9sQIB3Wtn', 'LEAP Staff', 'reports-distribution', 'read'),
  ('lE3NoPqRsTuVwXyZaBc3d', 'LEAP Staff', 'mails', 'read'),
  ('NVhNaVu62wK2bF0zK9oUD', 'Human Resource Staff', 'student-feedback-questions', 'read'),
  ('baqueKjBTS4WAqoACGOe_', 'Human Resource Staff', 'student-feedback-questions', 'create'),
  ('FA2WmIxh3vh1zga30nrEh', 'Human Resource Staff', 'student-feedback-questions', 'update'),
  ('gTPEYgxCeSSO39mGvVlqT', 'Human Resource Staff', 'student-feedback-questions', 'delete'),
  ('vvEjyOR9z7-n8FZojm_IG', 'Human Resource Staff', 'student-feedback-categories', 'read'),
  ('Mw0dGz1fxyJMGVFMNQ1QT', 'Human Resource Staff', 'student-feedback-categories', 'create'),
  ('eyMmTD38gR61YzQI_K7dj', 'Human Resource Staff', 'student-feedback-categories', 'update'),
  ('t7J0WrJoqck7Hcf5ggURS', 'Human Resource Staff', 'student-feedback-categories', 'delete'),
  ('q3NotDO-0LMwCwrKqWoQn', 'Human Resource Staff', 'feedback-cycles', 'read'),
  ('dGfUpdqjsigYzCWtq_qiO', 'Human Resource Staff', 'feedback-cycles', 'create'),
  ('gYSXHjoJefqMpNMSIWngH', 'Human Resource Staff', 'feedback-cycles', 'update'),
  ('F2Hjz-s1YFXApTkLL3ZaY', 'Human Resource Staff', 'student-feedback-reports', 'read'),
  ('fl9P3laaun1_geFIRKpJe', 'Human Resource Staff', 'student-feedback-reports', 'update'),
  ('r8O14DG0JM94XHr7Fl0Ot', 'Human Resource Staff', 'users', 'read'),
  ('t7N58BE4HK38VFp1Dj4Mr', 'Human Resource Staff', 'tasks', 'read'),
  ('u8O69CF5IL49WGq2Ek5Ns', 'Human Resource Staff', 'tasks', 'create'),
  ('v9P70DG6JM50XHr3Fl6Ot', 'Human Resource Staff', 'tasks', 'update'),
  ('w0Q81EH7KN61YIs4Gm7Pu', 'Human Resource Staff', 'tasks', 'delete'),
  ('9NlBfNO-p6xfZAdaI6_9f', 'Human Resource Staff', 'employees', 'read'),
  ('6Wq8ciR6_DGY980937B1u', 'Human Resource Staff', 'employees', 'create'),
  ('ZmptM5iqQvczo6uhvVj0Q', 'Human Resource Staff', 'employees', 'update'),
  ('srcf-Bvw_VVIdC89Z71kZ', 'Human Resource Staff', 'employees', 'delete'),
  ('f4kaVFaKEyM5PMEV-ymrs', 'Human Resource Staff', 'teaching-observations', 'read'),
  ('8_j2UZFMFfqjYa6juU8ae', 'Human Resource Staff', 'teaching-observation-criteria', 'read'),
  ('6ydeca_W-v8aY1_TZfWwR', 'Human Resource Staff', 'teaching-observation-criteria', 'create'),
  ('KLdJWZ2VSoKaSe4wYSiqo', 'Human Resource Staff', 'teaching-observation-criteria', 'update'),
  ('SwxrQ_1bm0IEznFRhb-KZ', 'Human Resource Staff', 'teaching-observation-criteria', 'delete'),
  ('ZwcY-vbq3yhZXrmZlCdhx', 'Human Resource Staff', 'teaching-observation-reports', 'read'),
  ('hR4OpQrStUvWxYzAbCd4e', 'Human Resource Staff', 'mails', 'read'),
  ('YAeGxyvoI4aJoSEn6RqO5', 'Resource Staff', 'timetable', 'read'),
  ('1pI0p1VE-eV0o128fd_7A', 'Resource Staff', 'venues', 'read'),
    ('UhE8v7dBl-WYgQ2DbC9pR', 'Resource Staff', 'registration', 'read'),
  ('eRL_FKQ-Odac8AAHu7ObN', 'Resource Staff', 'registration-clearance', 'read'),
  ('EZ2gOLQHW6f8qdxEozYbL', 'Resource Staff', 'registration-clearance', 'approve'),
  ('ZALo9mQZNsFzojWFzVKAE', 'Resource Staff', 'registration-clearance', 'reject'),
  ('x1R92FI8LO72ZJt5Hn8Qv', 'Resource Staff', 'tasks', 'read'),
  ('y2S03GJ9MP83AKu6Io9Rw', 'Resource Staff', 'tasks', 'create'),
  ('z3T14HK0NQ94BLv7Jp0Sx', 'Resource Staff', 'tasks', 'update'),
  ('04U25IL1OR05CMw8Kq1Ty', 'Resource Staff', 'tasks', 'delete'),
  ('kTxYAk6nD-g2PRPqQeoyv', 'Resource Staff', 'auto-approvals', 'read'),
  ('wa5q3aePROEUiId2m8Jbz', 'Resource Staff', 'auto-approvals', 'create'),
  ('_J-ndkLRr55yMJoUzwv98', 'Resource Staff', 'auto-approvals', 'update'),
  ('tSjuizrcTACXajOAzwzzZ', 'Resource Staff', 'auto-approvals', 'delete'),
  ('rE5PqRsTuVwXyZaBcDe5f', 'Resource Staff', 'mails', 'read')
) AS seed(id, preset_name, resource, action)
JOIN permission_presets AS preset ON preset.name = seed.preset_name
ON CONFLICT (preset_id, resource, action) DO NOTHING;
--> statement-breakpoint
UPDATE users AS user_record
SET preset_id = preset.id
FROM permission_presets AS preset
WHERE user_record.role = 'academic'
  AND user_record.position = 'manager'
  AND preset.name = 'Academic Manager'
  AND user_record.preset_id IS DISTINCT FROM preset.id;
--> statement-breakpoint
UPDATE users AS user_record
SET preset_id = preset.id
FROM permission_presets AS preset
WHERE user_record.role = 'academic'
  AND user_record.position = 'program_leader'
  AND preset.name = 'Program Leader'
  AND user_record.preset_id IS DISTINCT FROM preset.id;
--> statement-breakpoint
UPDATE users AS user_record
SET preset_id = preset.id
FROM permission_presets AS preset
WHERE user_record.role = 'academic'
  AND user_record.position = 'year_leader'
  AND preset.name = 'Year Leader'
  AND user_record.preset_id IS DISTINCT FROM preset.id;
--> statement-breakpoint
UPDATE users AS user_record
SET preset_id = preset.id
FROM permission_presets AS preset
WHERE user_record.role = 'academic'
  AND user_record.position = 'lecturer'
  AND preset.name = 'Lecturer'
  AND user_record.preset_id IS DISTINCT FROM preset.id;
--> statement-breakpoint
UPDATE users AS user_record
SET preset_id = preset.id
FROM permission_presets AS preset
WHERE user_record.role = 'academic'
  AND user_record.position = 'principal_lecturer'
  AND preset.name = 'Principal Lecturer'
  AND user_record.preset_id IS DISTINCT FROM preset.id;
--> statement-breakpoint
UPDATE users AS user_record
SET preset_id = preset.id
FROM permission_presets AS preset
WHERE user_record.role = 'academic'
  AND user_record.position = 'admin'
  AND preset.name = 'Academic Admin'
  AND user_record.preset_id IS DISTINCT FROM preset.id;
--> statement-breakpoint
UPDATE users AS user_record
SET preset_id = preset.id
FROM permission_presets AS preset
WHERE user_record.role = 'academic'
  AND user_record.position IS NULL
  AND preset.name = 'Lecturer'
  AND user_record.preset_id IS DISTINCT FROM preset.id;
--> statement-breakpoint
UPDATE users AS user_record
SET preset_id = preset.id
FROM permission_presets AS preset
WHERE user_record.role = 'registry'
  AND user_record.position IS NULL
  AND preset.name = 'Registry Staff'
  AND user_record.preset_id IS DISTINCT FROM preset.id;
--> statement-breakpoint
UPDATE users AS user_record
SET preset_id = preset.id
FROM permission_presets AS preset
WHERE user_record.role = 'registry'
  AND user_record.position = 'manager'
  AND preset.name = 'Registry Manager'
  AND user_record.preset_id IS DISTINCT FROM preset.id;
--> statement-breakpoint
UPDATE users AS user_record
SET preset_id = preset.id
FROM permission_presets AS preset
WHERE user_record.role = 'finance'
  AND user_record.position IS NULL
  AND preset.name = 'Finance Staff'
  AND user_record.preset_id IS DISTINCT FROM preset.id;
--> statement-breakpoint
UPDATE users AS user_record
SET preset_id = preset.id
FROM permission_presets AS preset
WHERE user_record.role = 'finance'
  AND user_record.position = 'manager'
  AND preset.name = 'Finance Manager'
  AND user_record.preset_id IS DISTINCT FROM preset.id;
--> statement-breakpoint
UPDATE users AS user_record
SET preset_id = preset.id
FROM permission_presets AS preset
WHERE user_record.role = 'library'
  AND preset.name = 'Library Staff'
  AND user_record.preset_id IS DISTINCT FROM preset.id;
--> statement-breakpoint
UPDATE users AS user_record
SET preset_id = preset.id
FROM permission_presets AS preset
WHERE user_record.role = 'marketing'
  AND preset.name = 'Marketing Staff'
  AND user_record.preset_id IS DISTINCT FROM preset.id;
--> statement-breakpoint
UPDATE users AS user_record
SET preset_id = preset.id
FROM permission_presets AS preset
WHERE user_record.role = 'student_services'
  AND preset.name = 'Student Services Staff'
  AND user_record.preset_id IS DISTINCT FROM preset.id;
--> statement-breakpoint
UPDATE users AS user_record
SET preset_id = preset.id
FROM permission_presets AS preset
WHERE user_record.role = 'leap'
  AND preset.name = 'LEAP Staff'
  AND user_record.preset_id IS DISTINCT FROM preset.id;
--> statement-breakpoint
UPDATE users AS user_record
SET preset_id = preset.id
FROM permission_presets AS preset
WHERE user_record.role = 'human_resource'
  AND preset.name = 'Human Resource Staff'
  AND user_record.preset_id IS DISTINCT FROM preset.id;
--> statement-breakpoint
UPDATE users AS user_record
SET preset_id = preset.id
FROM permission_presets AS preset
WHERE user_record.role = 'resource'
  AND preset.name = 'Resource Staff'
  AND user_record.preset_id IS DISTINCT FROM preset.id;
--> statement-breakpoint
DO $$
DECLARE
	v_seeded_preset_count INTEGER;
	v_seeded_permission_count INTEGER;
  v_expected_permission_count INTEGER;
	v_unmapped_count INTEGER;
	v_unmapped_summary TEXT;
BEGIN
	SELECT count(*) INTO v_seeded_preset_count FROM permission_presets WHERE name IN ('Academic Manager', 'Program Leader', 'Year Leader', 'Lecturer', 'Principal Lecturer', 'Academic Admin', 'Registry Staff', 'Registry Manager', 'Finance Staff', 'Finance Manager', 'Library Staff', 'Marketing Staff', 'Student Services Staff', 'LEAP Staff', 'Human Resource Staff', 'Resource Staff');
	IF v_seeded_preset_count != 16 THEN
		RAISE EXCEPTION 'ABORT: expected % seeded presets but found %',
			16, v_seeded_preset_count;
	END IF;

  WITH expected AS (
		VALUES
			('Academic Manager', 'lecturers', 'read'),
			('Academic Manager', 'lecturers', 'create'),
			('Academic Manager', 'lecturers', 'update'),
			('Academic Manager', 'lecturers', 'delete'),
			('Academic Manager', 'assessments', 'read'),
			('Academic Manager', 'assessments', 'create'),
			('Academic Manager', 'assessments', 'update'),
			('Academic Manager', 'assessments', 'delete'),
			('Academic Manager', 'student-feedback-questions', 'read'),
			('Academic Manager', 'student-feedback-questions', 'create'),
			('Academic Manager', 'student-feedback-questions', 'update'),
			('Academic Manager', 'student-feedback-questions', 'delete'),
			('Academic Manager', 'student-feedback-categories', 'read'),
			('Academic Manager', 'student-feedback-categories', 'create'),
			('Academic Manager', 'student-feedback-categories', 'update'),
			('Academic Manager', 'student-feedback-categories', 'delete'),
			('Academic Manager', 'feedback-cycles', 'read'),
			('Academic Manager', 'feedback-cycles', 'create'),
			('Academic Manager', 'feedback-cycles', 'update'),
			('Academic Manager', 'student-feedback-reports', 'read'),
			('Academic Manager', 'student-feedback-reports', 'update'),
			('Academic Manager', 'school-structures', 'read'),
			('Academic Manager', 'school-structures', 'update'),
			('Academic Manager', 'school-structures', 'delete'),
			('Academic Manager', 'timetable', 'read'),
			('Academic Manager', 'timetable', 'create'),
			('Academic Manager', 'timetable', 'update'),
			('Academic Manager', 'timetable', 'delete'),
			('Academic Manager', 'venues', 'read'),
			('Academic Manager', 'venues', 'create'),
			('Academic Manager', 'venues', 'update'),
			('Academic Manager', 'venues', 'delete'),
			('Academic Manager', 'students', 'read'),
			('Academic Manager', 'registration', 'read'),
			('Academic Manager', 'registration', 'update'),
      ('Academic Manager', 'registration-clearance', 'read'),
			('Academic Manager', 'graduation', 'read'),
			('Academic Manager', 'graduation', 'approve'),
			('Academic Manager', 'graduation-clearance', 'read'),
			('Academic Manager', 'graduation-clearance', 'approve'),
			('Academic Manager', 'graduation-clearance', 'reject'),
			('Academic Manager', 'activity-tracker', 'read'),
			('Academic Manager', 'tasks', 'read'),
			('Academic Manager', 'tasks', 'create'),
			('Academic Manager', 'tasks', 'update'),
			('Academic Manager', 'tasks', 'delete'),
			('Academic Manager', 'gradebook', 'read'),
			('Academic Manager', 'gradebook', 'update'),
			('Academic Manager', 'gradebook', 'approve'),
			('Academic Manager', 'attendance', 'read'),
			('Academic Manager', 'attendance', 'create'),
			('Academic Manager', 'attendance', 'update'),
			('Academic Manager', 'attendance', 'delete'),
			('Academic Manager', 'assigned-modules', 'read'),
			('Academic Manager', 'assigned-modules', 'create'),
			('Academic Manager', 'assigned-modules', 'delete'),
			('Academic Manager', 'reports-attendance', 'read'),
			('Academic Manager', 'reports-course-summary', 'read'),
			('Academic Manager', 'reports-boe', 'read'),
			('Academic Manager', 'reports-enrollments', 'read'),
			('Academic Manager', 'reports-progression', 'read'),
			('Academic Manager', 'reports-distribution', 'read'),
			('Academic Manager', 'reports-graduation', 'read'),
			('Academic Manager', 'teaching-observations', 'read'),
			('Academic Manager', 'teaching-observations', 'update'),
			('Academic Manager', 'teaching-observation-criteria', 'read'),
			('Academic Manager', 'teaching-observation-criteria', 'create'),
			('Academic Manager', 'teaching-observation-criteria', 'update'),
			('Academic Manager', 'teaching-observation-criteria', 'delete'),
			('Academic Manager', 'teaching-observation-reports', 'read'),
			('Academic Manager', 'student-statuses', 'approve'),
			('Academic Manager', 'mails', 'read'),
			('Program Leader', 'lecturers', 'read'),
			('Program Leader', 'lecturers', 'create'),
			('Program Leader', 'lecturers', 'update'),
			('Program Leader', 'lecturers', 'delete'),
			('Program Leader', 'student-feedback-questions', 'read'),
			('Program Leader', 'student-feedback-questions', 'create'),
			('Program Leader', 'student-feedback-questions', 'update'),
			('Program Leader', 'student-feedback-questions', 'delete'),
			('Program Leader', 'student-feedback-categories', 'read'),
			('Program Leader', 'student-feedback-categories', 'create'),
			('Program Leader', 'student-feedback-categories', 'update'),
			('Program Leader', 'student-feedback-categories', 'delete'),
			('Program Leader', 'school-structures', 'read'),
			('Program Leader', 'school-structures', 'update'),
			('Program Leader', 'registration', 'read'),
			('Program Leader', 'registration', 'update'),
      ('Program Leader', 'registration-clearance', 'read'),
			('Program Leader', 'timetable', 'read'),
			('Program Leader', 'students', 'read'),
			('Program Leader', 'gradebook', 'read'),
			('Program Leader', 'gradebook', 'update'),
			('Program Leader', 'gradebook', 'approve'),
			('Program Leader', 'attendance', 'read'),
			('Program Leader', 'assigned-modules', 'read'),
			('Program Leader', 'assigned-modules', 'create'),
			('Program Leader', 'assigned-modules', 'delete'),
      ('Program Leader', 'tasks', 'read'),
      ('Program Leader', 'tasks', 'create'),
      ('Program Leader', 'tasks', 'update'),
      ('Program Leader', 'tasks', 'delete'),
			('Program Leader', 'reports-attendance', 'read'),
			('Program Leader', 'reports-course-summary', 'read'),
			('Program Leader', 'reports-boe', 'read'),
			('Program Leader', 'reports-enrollments', 'read'),
			('Program Leader', 'reports-progression', 'read'),
			('Program Leader', 'reports-distribution', 'read'),
			('Program Leader', 'reports-graduation', 'read'),
			('Program Leader', 'teaching-observations', 'read'),
			('Program Leader', 'teaching-observations', 'create'),
			('Program Leader', 'teaching-observations', 'update'),
			('Program Leader', 'teaching-observations', 'delete'),
			('Program Leader', 'teaching-observation-criteria', 'read'),
			('Program Leader', 'teaching-observation-reports', 'read'),
			('Program Leader', 'student-statuses', 'approve'),
			('Program Leader', 'mails', 'read'),
			('Year Leader', 'feedback-cycles', 'read'),
			('Year Leader', 'registration', 'read'),
      ('Year Leader', 'registration-clearance', 'read'),
			('Year Leader', 'students', 'read'),
			('Year Leader', 'timetable', 'read'),
			('Year Leader', 'attendance', 'read'),
      ('Year Leader', 'tasks', 'read'),
      ('Year Leader', 'tasks', 'create'),
      ('Year Leader', 'tasks', 'update'),
      ('Year Leader', 'tasks', 'delete'),
			('Year Leader', 'reports-attendance', 'read'),
			('Year Leader', 'reports-enrollments', 'read'),
			('Year Leader', 'reports-progression', 'read'),
			('Year Leader', 'reports-distribution', 'read'),
			('Year Leader', 'teaching-observations', 'read'),
			('Year Leader', 'student-statuses', 'approve'),
			('Year Leader', 'mails', 'read'),
			('Lecturer', 'assessments', 'read'),
      ('Lecturer', 'registration', 'read'),
      ('Lecturer', 'registration-clearance', 'read'),
			('Lecturer', 'assessments', 'create'),
			('Lecturer', 'assessments', 'update'),
			('Lecturer', 'assessments', 'delete'),
			('Lecturer', 'gradebook', 'read'),
			('Lecturer', 'gradebook', 'update'),
			('Lecturer', 'student-feedback-reports', 'read'),
			('Lecturer', 'timetable', 'read'),
			('Lecturer', 'attendance', 'read'),
			('Lecturer', 'attendance', 'create'),
			('Lecturer', 'attendance', 'update'),
			('Lecturer', 'assigned-modules', 'read'),
      ('Lecturer', 'tasks', 'read'),
      ('Lecturer', 'tasks', 'create'),
      ('Lecturer', 'tasks', 'update'),
      ('Lecturer', 'tasks', 'delete'),
			('Lecturer', 'reports-attendance', 'read'),
			('Lecturer', 'reports-course-summary', 'read'),
			('Lecturer', 'teaching-observations', 'read'),
			('Lecturer', 'mails', 'read'),
			('Principal Lecturer', 'assessments', 'read'),
      ('Principal Lecturer', 'registration', 'read'),
      ('Principal Lecturer', 'registration-clearance', 'read'),
			('Principal Lecturer', 'assessments', 'create'),
			('Principal Lecturer', 'assessments', 'update'),
			('Principal Lecturer', 'assessments', 'delete'),
			('Principal Lecturer', 'gradebook', 'read'),
			('Principal Lecturer', 'gradebook', 'update'),
			('Principal Lecturer', 'gradebook', 'approve'),
			('Principal Lecturer', 'student-feedback-reports', 'read'),
			('Principal Lecturer', 'timetable', 'read'),
			('Principal Lecturer', 'attendance', 'read'),
			('Principal Lecturer', 'attendance', 'create'),
			('Principal Lecturer', 'attendance', 'update'),
			('Principal Lecturer', 'assigned-modules', 'read'),
      ('Principal Lecturer', 'tasks', 'read'),
      ('Principal Lecturer', 'tasks', 'create'),
      ('Principal Lecturer', 'tasks', 'update'),
      ('Principal Lecturer', 'tasks', 'delete'),
			('Principal Lecturer', 'reports-attendance', 'read'),
			('Principal Lecturer', 'reports-course-summary', 'read'),
			('Principal Lecturer', 'teaching-observations', 'read'),
			('Principal Lecturer', 'teaching-observation-reports', 'read'),
			('Principal Lecturer', 'mails', 'read'),
			('Academic Admin', 'assessments', 'read'),
      ('Academic Admin', 'registration', 'read'),
      ('Academic Admin', 'registration-clearance', 'read'),
			('Academic Admin', 'assessments', 'create'),
			('Academic Admin', 'assessments', 'update'),
			('Academic Admin', 'assessments', 'delete'),
			('Academic Admin', 'student-feedback-questions', 'read'),
			('Academic Admin', 'student-feedback-questions', 'create'),
			('Academic Admin', 'student-feedback-questions', 'update'),
			('Academic Admin', 'student-feedback-questions', 'delete'),
			('Academic Admin', 'student-feedback-categories', 'read'),
			('Academic Admin', 'student-feedback-categories', 'create'),
			('Academic Admin', 'student-feedback-categories', 'update'),
			('Academic Admin', 'student-feedback-categories', 'delete'),
			('Academic Admin', 'feedback-cycles', 'read'),
			('Academic Admin', 'feedback-cycles', 'create'),
			('Academic Admin', 'feedback-cycles', 'update'),
			('Academic Admin', 'timetable', 'read'),
      ('Academic Admin', 'tasks', 'read'),
      ('Academic Admin', 'tasks', 'create'),
      ('Academic Admin', 'tasks', 'update'),
      ('Academic Admin', 'tasks', 'delete'),
			('Academic Admin', 'attendance', 'read'),
			('Academic Admin', 'teaching-observations', 'read'),
			('Academic Admin', 'teaching-observation-criteria', 'read'),
			('Academic Admin', 'teaching-observation-criteria', 'create'),
			('Academic Admin', 'teaching-observation-criteria', 'update'),
			('Academic Admin', 'teaching-observation-criteria', 'delete'),
			('Academic Admin', 'teaching-observation-reports', 'read'),
			('Academic Admin', 'mails', 'read'),
			('Registry Staff', 'students', 'read'),
			('Registry Staff', 'students', 'update'),
			('Registry Staff', 'students', 'delete'),
      ('Registry Staff', 'sponsored-students', 'read'),
      ('Registry Staff', 'sponsored-students', 'create'),
      ('Registry Staff', 'sponsored-students', 'update'),
      ('Registry Staff', 'sponsored-students', 'delete'),
			('Registry Staff', 'registration', 'read'),
			('Registry Staff', 'registration', 'create'),
			('Registry Staff', 'registration', 'update'),
			('Registry Staff', 'registration', 'delete'),
			('Registry Staff', 'registration-clearance', 'read'),
      ('Registry Staff', 'applicants', 'read'),
      ('Registry Staff', 'applicants', 'create'),
      ('Registry Staff', 'applicants', 'update'),
      ('Registry Staff', 'applicants', 'delete'),
      ('Registry Staff', 'applications', 'read'),
      ('Registry Staff', 'applications', 'create'),
      ('Registry Staff', 'applications', 'update'),
      ('Registry Staff', 'applications', 'delete'),
      ('Registry Staff', 'admissions-documents', 'read'),
      ('Registry Staff', 'admissions-documents', 'create'),
      ('Registry Staff', 'admissions-documents', 'update'),
      ('Registry Staff', 'admissions-documents', 'delete'),
      ('Registry Staff', 'admissions-documents', 'approve'),
      ('Registry Staff', 'admissions-documents', 'reject'),
      ('Registry Staff', 'entry-requirements', 'read'),
      ('Registry Staff', 'entry-requirements', 'create'),
      ('Registry Staff', 'entry-requirements', 'update'),
      ('Registry Staff', 'entry-requirements', 'delete'),
      ('Registry Staff', 'recognized-schools', 'read'),
      ('Registry Staff', 'recognized-schools', 'create'),
      ('Registry Staff', 'recognized-schools', 'update'),
      ('Registry Staff', 'recognized-schools', 'delete'),
      ('Registry Staff', 'intake-periods', 'read'),
      ('Registry Staff', 'intake-periods', 'create'),
      ('Registry Staff', 'intake-periods', 'update'),
      ('Registry Staff', 'intake-periods', 'delete'),
      ('Registry Staff', 'certificate-types', 'read'),
      ('Registry Staff', 'certificate-types', 'create'),
      ('Registry Staff', 'certificate-types', 'update'),
      ('Registry Staff', 'certificate-types', 'delete'),
      ('Registry Staff', 'subjects', 'read'),
      ('Registry Staff', 'subjects', 'create'),
      ('Registry Staff', 'subjects', 'update'),
      ('Registry Staff', 'subjects', 'delete'),
			('Registry Staff', 'documents', 'read'),
			('Registry Staff', 'documents', 'create'),
			('Registry Staff', 'student-statuses', 'read'),
			('Registry Staff', 'student-statuses', 'create'),
			('Registry Staff', 'student-statuses', 'update'),
			('Registry Staff', 'student-statuses', 'delete'),
			('Registry Staff', 'terms-settings', 'read'),
			('Registry Staff', 'terms-settings', 'update'),
			('Registry Staff', 'certificate-reprints', 'read'),
			('Registry Staff', 'certificate-reprints', 'create'),
			('Registry Staff', 'certificate-reprints', 'update'),
			('Registry Staff', 'certificate-reprints', 'delete'),
			('Registry Staff', 'modules', 'create'),
			('Registry Staff', 'semester-modules', 'create'),
			('Registry Staff', 'semester-modules', 'update'),
			('Registry Staff', 'venues', 'create'),
			('Registry Staff', 'venues', 'update'),
			('Registry Staff', 'venues', 'delete'),
			('Registry Staff', 'graduation', 'read'),
      ('Registry Staff', 'tasks', 'read'),
      ('Registry Staff', 'tasks', 'create'),
      ('Registry Staff', 'tasks', 'update'),
      ('Registry Staff', 'tasks', 'delete'),
			('Registry Staff', 'student-notes', 'read'),
			('Registry Staff', 'student-notes', 'create'),
			('Registry Staff', 'student-notes', 'update'),
			('Registry Staff', 'student-notes', 'delete'),
			('Registry Staff', 'blocked-students', 'read'),
			('Registry Staff', 'blocked-students', 'create'),
			('Registry Staff', 'blocked-students', 'update'),
			('Registry Staff', 'reports-enrollments', 'read'),
			('Registry Staff', 'reports-progression', 'read'),
			('Registry Staff', 'reports-distribution', 'read'),
			('Registry Staff', 'reports-attendance', 'read'),
			('Registry Staff', 'mails', 'read'),
			('Registry Manager', 'students', 'read'),
			('Registry Manager', 'students', 'update'),
			('Registry Manager', 'students', 'delete'),
      ('Registry Manager', 'sponsors', 'read'),
      ('Registry Manager', 'sponsors', 'create'),
      ('Registry Manager', 'sponsors', 'update'),
      ('Registry Manager', 'sponsors', 'delete'),
      ('Registry Manager', 'sponsored-students', 'read'),
      ('Registry Manager', 'sponsored-students', 'create'),
      ('Registry Manager', 'sponsored-students', 'update'),
      ('Registry Manager', 'sponsored-students', 'delete'),
			('Registry Manager', 'registration', 'read'),
			('Registry Manager', 'registration', 'create'),
			('Registry Manager', 'registration', 'update'),
			('Registry Manager', 'registration', 'delete'),
			('Registry Manager', 'registration-clearance', 'read'),
      ('Registry Manager', 'applicants', 'read'),
      ('Registry Manager', 'applicants', 'create'),
      ('Registry Manager', 'applicants', 'update'),
      ('Registry Manager', 'applicants', 'delete'),
      ('Registry Manager', 'applications', 'read'),
      ('Registry Manager', 'applications', 'create'),
      ('Registry Manager', 'applications', 'update'),
      ('Registry Manager', 'applications', 'delete'),
      ('Registry Manager', 'admissions-documents', 'read'),
      ('Registry Manager', 'admissions-documents', 'create'),
      ('Registry Manager', 'admissions-documents', 'update'),
      ('Registry Manager', 'admissions-documents', 'delete'),
      ('Registry Manager', 'admissions-documents', 'approve'),
      ('Registry Manager', 'admissions-documents', 'reject'),
      ('Registry Manager', 'entry-requirements', 'read'),
      ('Registry Manager', 'entry-requirements', 'create'),
      ('Registry Manager', 'entry-requirements', 'update'),
      ('Registry Manager', 'entry-requirements', 'delete'),
      ('Registry Manager', 'recognized-schools', 'read'),
      ('Registry Manager', 'recognized-schools', 'create'),
      ('Registry Manager', 'recognized-schools', 'update'),
      ('Registry Manager', 'recognized-schools', 'delete'),
      ('Registry Manager', 'intake-periods', 'read'),
      ('Registry Manager', 'intake-periods', 'create'),
      ('Registry Manager', 'intake-periods', 'update'),
      ('Registry Manager', 'intake-periods', 'delete'),
      ('Registry Manager', 'certificate-types', 'read'),
      ('Registry Manager', 'certificate-types', 'create'),
      ('Registry Manager', 'certificate-types', 'update'),
      ('Registry Manager', 'certificate-types', 'delete'),
      ('Registry Manager', 'subjects', 'read'),
      ('Registry Manager', 'subjects', 'create'),
      ('Registry Manager', 'subjects', 'update'),
      ('Registry Manager', 'subjects', 'delete'),
			('Registry Manager', 'documents', 'read'),
			('Registry Manager', 'documents', 'create'),
			('Registry Manager', 'student-statuses', 'read'),
			('Registry Manager', 'student-statuses', 'create'),
			('Registry Manager', 'student-statuses', 'update'),
			('Registry Manager', 'student-statuses', 'delete'),
			('Registry Manager', 'terms-settings', 'read'),
			('Registry Manager', 'terms-settings', 'update'),
			('Registry Manager', 'certificate-reprints', 'read'),
			('Registry Manager', 'certificate-reprints', 'create'),
			('Registry Manager', 'certificate-reprints', 'update'),
			('Registry Manager', 'certificate-reprints', 'delete'),
			('Registry Manager', 'modules', 'create'),
			('Registry Manager', 'semester-modules', 'create'),
			('Registry Manager', 'semester-modules', 'update'),
			('Registry Manager', 'venues', 'create'),
			('Registry Manager', 'venues', 'update'),
			('Registry Manager', 'venues', 'delete'),
			('Registry Manager', 'graduation', 'read'),
			('Registry Manager', 'activity-tracker', 'read'),
			('Registry Manager', 'tasks', 'read'),
			('Registry Manager', 'tasks', 'create'),
			('Registry Manager', 'tasks', 'update'),
			('Registry Manager', 'tasks', 'delete'),
			('Registry Manager', 'school-structures', 'update'),
			('Registry Manager', 'student-notes', 'read'),
			('Registry Manager', 'student-notes', 'create'),
			('Registry Manager', 'student-notes', 'update'),
			('Registry Manager', 'student-notes', 'delete'),
			('Registry Manager', 'blocked-students', 'read'),
			('Registry Manager', 'blocked-students', 'create'),
			('Registry Manager', 'blocked-students', 'update'),
			('Registry Manager', 'notifications', 'read'),
			('Registry Manager', 'mails', 'read'),
			('Registry Manager', 'reports-enrollments', 'read'),
			('Registry Manager', 'reports-progression', 'read'),
			('Registry Manager', 'reports-distribution', 'read'),
			('Registry Manager', 'reports-attendance', 'read'),
			('Registry Manager', 'reports-graduation', 'read'),
			('Finance Staff', 'sponsors', 'read'),
			('Finance Staff', 'sponsors', 'create'),
			('Finance Staff', 'sponsors', 'update'),
			('Finance Staff', 'sponsors', 'delete'),
      ('Finance Staff', 'sponsored-students', 'read'),
      ('Finance Staff', 'sponsored-students', 'create'),
      ('Finance Staff', 'sponsored-students', 'update'),
      ('Finance Staff', 'sponsored-students', 'delete'),
      ('Finance Staff', 'zoho', 'read'),
      ('Finance Staff', 'zoho', 'create'),
      ('Finance Staff', 'zoho', 'update'),
      ('Finance Staff', 'zoho', 'delete'),
			('Finance Staff', 'admissions-payments', 'read'),
			('Finance Staff', 'admissions-payments', 'update'),
			('Finance Staff', 'student-statuses', 'read'),
			('Finance Staff', 'graduation', 'read'),
			('Finance Staff', 'students', 'read'),
      ('Finance Staff', 'registration', 'read'),
			('Finance Staff', 'registration-clearance', 'read'),
			('Finance Staff', 'registration-clearance', 'approve'),
			('Finance Staff', 'registration-clearance', 'reject'),
			('Finance Staff', 'graduation-clearance', 'read'),
			('Finance Staff', 'graduation-clearance', 'approve'),
			('Finance Staff', 'graduation-clearance', 'reject'),
			('Finance Staff', 'blocked-students', 'read'),
			('Finance Staff', 'blocked-students', 'create'),
			('Finance Staff', 'blocked-students', 'update'),
      ('Finance Staff', 'tasks', 'read'),
      ('Finance Staff', 'tasks', 'create'),
      ('Finance Staff', 'tasks', 'update'),
      ('Finance Staff', 'tasks', 'delete'),
			('Finance Staff', 'reports-enrollments', 'read'),
			('Finance Staff', 'reports-progression', 'read'),
			('Finance Staff', 'reports-distribution', 'read'),
			('Finance Staff', 'reports-graduation', 'read'),
			('Finance Staff', 'reports-sponsored-students', 'read'),
			('Finance Staff', 'student-statuses', 'approve'),
			('Finance Staff', 'mails', 'read'),
			('Finance Manager', 'sponsors', 'read'),
			('Finance Manager', 'sponsors', 'create'),
			('Finance Manager', 'sponsors', 'update'),
			('Finance Manager', 'sponsors', 'delete'),
      ('Finance Manager', 'sponsored-students', 'read'),
      ('Finance Manager', 'sponsored-students', 'create'),
      ('Finance Manager', 'sponsored-students', 'update'),
      ('Finance Manager', 'sponsored-students', 'delete'),
      ('Finance Manager', 'zoho', 'read'),
      ('Finance Manager', 'zoho', 'create'),
      ('Finance Manager', 'zoho', 'update'),
      ('Finance Manager', 'zoho', 'delete'),
			('Finance Manager', 'admissions-payments', 'read'),
			('Finance Manager', 'admissions-payments', 'update'),
			('Finance Manager', 'student-statuses', 'read'),
			('Finance Manager', 'graduation', 'read'),
			('Finance Manager', 'students', 'read'),
      ('Finance Manager', 'registration', 'read'),
			('Finance Manager', 'registration-clearance', 'read'),
			('Finance Manager', 'registration-clearance', 'approve'),
			('Finance Manager', 'registration-clearance', 'reject'),
			('Finance Manager', 'graduation-clearance', 'read'),
			('Finance Manager', 'graduation-clearance', 'approve'),
			('Finance Manager', 'graduation-clearance', 'reject'),
      ('Finance Manager', 'blocked-students', 'read'),
      ('Finance Manager', 'blocked-students', 'create'),
      ('Finance Manager', 'blocked-students', 'update'),
      ('Finance Staff', 'auto-approvals', 'read'),
      ('Finance Staff', 'auto-approvals', 'create'),
      ('Finance Staff', 'auto-approvals', 'update'),
      ('Finance Staff', 'auto-approvals', 'delete'),
      ('Finance Manager', 'auto-approvals', 'read'),
			('Finance Manager', 'auto-approvals', 'create'),
			('Finance Manager', 'auto-approvals', 'update'),
			('Finance Manager', 'auto-approvals', 'delete'),
			('Finance Manager', 'activity-tracker', 'read'),
			('Finance Manager', 'tasks', 'read'),
			('Finance Manager', 'tasks', 'create'),
			('Finance Manager', 'tasks', 'update'),
			('Finance Manager', 'tasks', 'delete'),
			('Finance Manager', 'reports-enrollments', 'read'),
			('Finance Manager', 'reports-progression', 'read'),
			('Finance Manager', 'reports-distribution', 'read'),
			('Finance Manager', 'reports-graduation', 'read'),
			('Finance Manager', 'reports-sponsored-students', 'read'),
			('Finance Manager', 'student-statuses', 'approve'),
			('Finance Manager', 'mails', 'read'),
			('Library Staff', 'library', 'read'),
			('Library Staff', 'library', 'create'),
			('Library Staff', 'library', 'update'),
			('Library Staff', 'library', 'delete'),
			('Library Staff', 'students', 'read'),
      ('Library Staff', 'registration', 'read'),
			('Library Staff', 'registration-clearance', 'read'),
			('Library Staff', 'registration-clearance', 'approve'),
			('Library Staff', 'registration-clearance', 'reject'),
			('Library Staff', 'graduation-clearance', 'read'),
			('Library Staff', 'graduation-clearance', 'approve'),
			('Library Staff', 'graduation-clearance', 'reject'),
      ('Library Staff', 'tasks', 'read'),
      ('Library Staff', 'tasks', 'create'),
      ('Library Staff', 'tasks', 'update'),
      ('Library Staff', 'tasks', 'delete'),
			('Library Staff', 'auto-approvals', 'read'),
			('Library Staff', 'auto-approvals', 'create'),
			('Library Staff', 'auto-approvals', 'update'),
			('Library Staff', 'auto-approvals', 'delete'),
			('Library Staff', 'mails', 'read'),
			('Marketing Staff', 'applicants', 'read'),
			('Marketing Staff', 'applicants', 'create'),
			('Marketing Staff', 'applicants', 'update'),
			('Marketing Staff', 'applicants', 'delete'),
			('Marketing Staff', 'applications', 'read'),
			('Marketing Staff', 'applications', 'create'),
			('Marketing Staff', 'applications', 'update'),
			('Marketing Staff', 'applications', 'delete'),
			('Marketing Staff', 'entry-requirements', 'read'),
			('Marketing Staff', 'entry-requirements', 'create'),
			('Marketing Staff', 'entry-requirements', 'update'),
			('Marketing Staff', 'entry-requirements', 'delete'),
			('Marketing Staff', 'admissions-payments', 'read'),
			('Marketing Staff', 'admissions-documents', 'read'),
      ('Marketing Staff', 'admissions-documents', 'update'),
      ('Marketing Staff', 'admissions-documents', 'approve'),
      ('Marketing Staff', 'admissions-documents', 'reject'),
			('Marketing Staff', 'recognized-schools', 'read'),
			('Marketing Staff', 'recognized-schools', 'create'),
			('Marketing Staff', 'recognized-schools', 'update'),
			('Marketing Staff', 'recognized-schools', 'delete'),
			('Marketing Staff', 'intake-periods', 'read'),
			('Marketing Staff', 'intake-periods', 'create'),
			('Marketing Staff', 'intake-periods', 'update'),
			('Marketing Staff', 'intake-periods', 'delete'),
			('Marketing Staff', 'certificate-types', 'read'),
			('Marketing Staff', 'certificate-types', 'create'),
			('Marketing Staff', 'certificate-types', 'update'),
			('Marketing Staff', 'certificate-types', 'delete'),
      ('Marketing Staff', 'tasks', 'read'),
      ('Marketing Staff', 'tasks', 'create'),
      ('Marketing Staff', 'tasks', 'update'),
      ('Marketing Staff', 'tasks', 'delete'),
			('Marketing Staff', 'subjects', 'read'),
			('Marketing Staff', 'subjects', 'create'),
			('Marketing Staff', 'subjects', 'update'),
			('Marketing Staff', 'subjects', 'delete'),
			('Marketing Staff', 'mails', 'read'),
			('Student Services Staff', 'students', 'read'),
			('Student Services Staff', 'students', 'update'),
			('Student Services Staff', 'registration', 'read'),
			('Student Services Staff', 'registration', 'update'),
      ('Student Services Staff', 'registration-clearance', 'read'),
			('Student Services Staff', 'documents', 'read'),
			('Student Services Staff', 'documents', 'create'),
			('Student Services Staff', 'student-statuses', 'read'),
			('Student Services Staff', 'student-statuses', 'create'),
			('Student Services Staff', 'student-statuses', 'update'),
      ('Student Services Staff', 'tasks', 'read'),
      ('Student Services Staff', 'tasks', 'create'),
      ('Student Services Staff', 'tasks', 'update'),
      ('Student Services Staff', 'tasks', 'delete'),
			('Student Services Staff', 'student-notes', 'read'),
			('Student Services Staff', 'student-notes', 'create'),
			('Student Services Staff', 'student-notes', 'update'),
			('Student Services Staff', 'student-notes', 'delete'),
			('Student Services Staff', 'student-statuses', 'approve'),
			('Student Services Staff', 'mails', 'read'),
			('LEAP Staff', 'assessments', 'read'),
			('LEAP Staff', 'assessments', 'create'),
			('LEAP Staff', 'assessments', 'update'),
			('LEAP Staff', 'assessments', 'delete'),
			('LEAP Staff', 'registration', 'read'),
			('LEAP Staff', 'registration', 'update'),
			('LEAP Staff', 'registration-clearance', 'read'),
			('LEAP Staff', 'students', 'read'),
			('LEAP Staff', 'attendance', 'read'),
			('LEAP Staff', 'attendance', 'create'),
			('LEAP Staff', 'attendance', 'update'),
			('LEAP Staff', 'assigned-modules', 'read'),
      ('LEAP Staff', 'tasks', 'read'),
      ('LEAP Staff', 'tasks', 'create'),
      ('LEAP Staff', 'tasks', 'update'),
      ('LEAP Staff', 'tasks', 'delete'),
			('LEAP Staff', 'reports-attendance', 'read'),
			('LEAP Staff', 'reports-enrollments', 'read'),
			('LEAP Staff', 'reports-progression', 'read'),
			('LEAP Staff', 'reports-distribution', 'read'),
			('LEAP Staff', 'mails', 'read'),
			('Human Resource Staff', 'student-feedback-questions', 'read'),
			('Human Resource Staff', 'student-feedback-questions', 'create'),
			('Human Resource Staff', 'student-feedback-questions', 'update'),
			('Human Resource Staff', 'student-feedback-questions', 'delete'),
			('Human Resource Staff', 'student-feedback-categories', 'read'),
			('Human Resource Staff', 'student-feedback-categories', 'create'),
			('Human Resource Staff', 'student-feedback-categories', 'update'),
			('Human Resource Staff', 'student-feedback-categories', 'delete'),
			('Human Resource Staff', 'feedback-cycles', 'read'),
			('Human Resource Staff', 'feedback-cycles', 'create'),
			('Human Resource Staff', 'feedback-cycles', 'update'),
			('Human Resource Staff', 'student-feedback-reports', 'read'),
			('Human Resource Staff', 'student-feedback-reports', 'update'),
      ('Human Resource Staff', 'users', 'read'),
      ('Human Resource Staff', 'tasks', 'read'),
      ('Human Resource Staff', 'tasks', 'create'),
      ('Human Resource Staff', 'tasks', 'update'),
      ('Human Resource Staff', 'tasks', 'delete'),
			('Human Resource Staff', 'employees', 'read'),
			('Human Resource Staff', 'employees', 'create'),
			('Human Resource Staff', 'employees', 'update'),
			('Human Resource Staff', 'employees', 'delete'),
			('Human Resource Staff', 'teaching-observations', 'read'),
			('Human Resource Staff', 'teaching-observation-criteria', 'read'),
			('Human Resource Staff', 'teaching-observation-criteria', 'create'),
			('Human Resource Staff', 'teaching-observation-criteria', 'update'),
			('Human Resource Staff', 'teaching-observation-criteria', 'delete'),
			('Human Resource Staff', 'teaching-observation-reports', 'read'),
			('Human Resource Staff', 'mails', 'read'),
			('Resource Staff', 'timetable', 'read'),
			('Resource Staff', 'venues', 'read'),
      ('Resource Staff', 'registration', 'read'),
			('Resource Staff', 'registration-clearance', 'read'),
			('Resource Staff', 'registration-clearance', 'approve'),
			('Resource Staff', 'registration-clearance', 'reject'),
      ('Resource Staff', 'tasks', 'read'),
      ('Resource Staff', 'tasks', 'create'),
      ('Resource Staff', 'tasks', 'update'),
      ('Resource Staff', 'tasks', 'delete'),
			('Resource Staff', 'auto-approvals', 'read'),
			('Resource Staff', 'auto-approvals', 'create'),
      ('Resource Staff', 'auto-approvals', 'update'),
      ('Resource Staff', 'auto-approvals', 'delete'),
			('Resource Staff', 'mails', 'read')
  )
  SELECT count(*), (SELECT count(*) FROM expected)
  INTO v_seeded_permission_count, v_expected_permission_count
  FROM preset_permissions AS permission
  JOIN permission_presets AS preset ON preset.id = permission.preset_id
  JOIN expected AS expected(preset_name, resource, action)
    ON expected.preset_name = preset.name
    AND expected.resource = permission.resource
    AND expected.action = permission.action;
  IF v_seeded_permission_count != v_expected_permission_count THEN
		RAISE EXCEPTION 'ABORT: expected % seeded preset permissions but found %',
      v_expected_permission_count, v_seeded_permission_count;
	END IF;

	SELECT count(*) INTO v_unmapped_count
	FROM users
	WHERE preset_id IS NULL AND role NOT IN ('user', 'applicant', 'student', 'admin');

	IF v_unmapped_count > 0 THEN
		SELECT string_agg(
			format('%s | %s | %s', role, COALESCE(position, 'NULL'), affected_users),
			E'\n'
			ORDER BY role, position
		)
		INTO v_unmapped_summary
		FROM (
			SELECT role, position, count(*) AS affected_users
			FROM users
			WHERE preset_id IS NULL AND role NOT IN ('user', 'applicant', 'student', 'admin')
			GROUP BY role, position
		) AS unmapped;

		RAISE EXCEPTION 'ABORT: % unmapped staff users remain.%',
			v_unmapped_count, COALESCE(E'\n' || v_unmapped_summary, '');
	END IF;

	RAISE NOTICE 'Phase 6 seed verified: % presets, % preset permissions, 0 unmapped staff users.',
		v_seeded_preset_count, v_seeded_permission_count;
END $$;
